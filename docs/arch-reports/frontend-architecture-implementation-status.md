...
`frontend-architecture-fa-03-dispatcher-d3b-canary-secret-isolation-2026-09-07.md`。
...
| 目前批次                | dispatcher D3C 已新增 exact `database-probe`／`dispatch` 動作門檻；probe 只驗專用 role 連線、不建 worker／不寫 DB／不送 Push，並通過完整 CI。Hosted 仍只有 D1 migration，尚未設定 credential／Secret 或部署 canary。B13.3 UI 另等待產品決策                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation 與全部 39 份 Hosted migration 已完成；D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                                                                                                                                                                                  |
| runtime 變更            | Auth gate、current-device local sign-out、DB dormant command、本機 cleanup Edge、兩套獨立 public-key build boundary、dormant IndexedDB／cleanup transport／owner adapter／coordinator／Auth handoff／Push deactivation／manual re-enable coordinator／pre-network cancel／refresh commit／subscription browser／transport／local composition、dormant Push v2 validator／hybrid crypto／Edge ports、local-only Push v2 HTTP／Auth／DB handler、default-off app composition shell、dormant dispatcher egress／outcome core、local D1 DB barrier、local-only D2 Edge／DB／mock、D3A Deno-native sender 與 default-unavailable 獨立 canary，以及 local-only cleanup limiter composition 已落地；Push 登出 server cleanup、v2 UI、SW、Hosted dispatcher 尚未接線 |
...
| 下一步                  | 設定六個 canary-only Secret、只部署獨立 Function，再送一次 no-write `database-probe` 驗證 Hosted Edge 能以專用 role 直連 DB；不送 `dispatch`、不改 runtime／cron。真正 browser fixture、runtime policy、dispatch request、generation／legacy cutoff 再分批；B13.3 UI 另等待產品確認                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| D22 | provider egress 採應用層充分條件：exact provider-origin allowlist＋send-time DNS public-IP 檢查即可進 enabled；平台 egress 層 allowlist 為 nice-to-have，殘餘風險為攻擊者需控制 provider DNS 解析結果                                                                                                                                | 使用者於 2026-09-02 拍板；B11.1 把 allowlist 保留在 server；D3A 已完成 local send-time DNS／IP pinning，Hosted provider secret、部署與 canary 尚未執行                                                        |
...
| D27 | Push v2 provider policy 不內建猜測的 production hostname；設定只接受 sorted／unique／non-empty canonical origin JSON，endpoint 名稱再依 IANA Special-Use registry fail-closed                                                                                                                                                        | B11.1 已拆分 browser／server policy；D3A local send-time DNS 已完成；Hosted 現有 Push endpoint 唯讀彙總只有 exact FCM origin，但 provider secret 尚未設定，正式值仍須在設定前重查                             |
...
| D34 | D2 direct DB client 採 exact `jsr:@db/postgres@0.19.5` 與 checked-out `PoolClient`；prepare、provider request、complete 必須在同一筆 transaction。只提供 local exact mode，不能使用 Hosted marker 或正式 mock transport                                                                                                              | active repo source 與真實 local Edge／DB／mock composition 已完成；D3A sender 也已走同一 transaction 完成 accepted；Hosted D1 migration 已套用，credential／deploy／control／canary 均未執行                  |
| D35 | Hosted dispatcher canary 使用獨立、預設 unavailable 的 Function 與 manual-run secret，不改現行 cron 或先部署 active dispatcher；token 本身不宣稱 cryptographic one-shot，request 數由核可操作流程限制                                                                                                                                | repo／local source 與 2 條 integration 已完成，Hosted D1 migration 也已套用；Function／secret／request／runtime policy 另行確認                                                                               |
| D36 | 本專案所有 migration 已取得持續授權，不必逐支再詢問；每次仍要先 dry-run、限制 exact scope、套用後驗證並記錄。這不包含 deploy、secret／credential、runtime control、request、資料清除或 legacy cutoff                                                                                                                                 | 使用者於 2026-09-07 明確確認；D1 migration 已依此套用，39／39、資料／catalog／lint／diff 驗證完成                                                                                                             |
| D37 | 獨立 Hosted canary 的 DB URL／generation／provider policy／transport 必須使用 canary-only Secret 名稱；只共享既有 VAPID，避免 active dispatcher 意外繼承 canary 設定                                                                                                                                                                 | D3B repo／local 已完成；unit 4／4、local sender／DB 2／2 與完整 CI 通過，Hosted 尚未設定或部署                                                                                                                |
| D38 | Hosted canary request 必須明確指定 exact action；`database-probe` 只驗證專用 role 連線並回固定 ready，不建立 worker／不寫 DB／不送 Push；只有 `dispatch` 能進派送流程                                                                                                                                                                | D3C repo／local 已完成；unit 5／5、local probe 後 worker 仍為 0、sender／DB 2／2 與完整 CI 通過；Hosted 尚未設定、部署或呼叫                                                                                  |
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   D1 Hosted migration 套用都已完成。下一步是另行確認 DB credential／secret／獨立 canary deploy；control write 與
   canary request 仍在後續批次。
