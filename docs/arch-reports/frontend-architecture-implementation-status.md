...
| 目前批次                | Hosted source trust 通過並復原：單一已授權空 request 精確回 503／RETRY／`POLICY`，Function exact-path log 1、limiter RPC log 0；所有 mutable DB 為 0，既有 Function hash 與其他基線不變                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| 下一步                  | 先提交 no-policy Hosted 結果文件；下一批只做真正 Hosted limiter canary 的唯讀／本機 preflight，固定 canary-only policy、request 上限、預期 row 與 guarded cleanup，不能把測試值當 production policy                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
