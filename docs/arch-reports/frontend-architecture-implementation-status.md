...
| 目前批次                | `FA-06 stage 4.3` 完成：4 個 configure 與 14 個 unmount registrations 已搬到 `src/views/sessionViewWiring.js`；`main.js` 明確啟動一次並鎖定 discovery → session → profile → form 順序。私有 disclosure／form runtime 跟著真實 owner 搬移；完整 frontend CI 與 production preview 通過                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3`、`FA-06 stage 4` preflight／4.1／4.2／4.3 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                                                              |
...
| 下一步                  | 執行 `FA-06 stage 4.4`：把 app-module wiring、authenticated preload state 與 2 個 intent listeners 搬到專用 wiring owner，安裝函式必須可重複呼叫但不重複註冊，並依 AST 實掃更新 browser port manifest；`sessionViews` facade 與 86 個 harness calls 暫時保留。Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| FA-06 | `sessionViews` wiring、blockedPlayers、Chat／Messages ownership   | 階段 4 preflight／4.1 tests-only gate manifest 化／4.2 loader／4.3 configure wiring 完成；下一步為 4.4 app module 與 listeners                                                                                                                                                                                                                                         | 每個新 owner 都伴隨舊 bridge 刪除與完整回歸                                                                                                                                                               |
...
- 最新一般 development build 的 main 為 650,113／191,175／159,810，total 為
  852,737／261,314／220,788 raw／gzip／Brotli；raw／gzip 比目前參考值多 2,776／2,252 bytes。D8 允許開發期
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   FA-06 階段 4 preflight、4.1 tests-only gate manifest 化、4.2 `surfaceLoaders.js` 與 4.3 configure wiring 已完成；
   下一步做 4.4 app-module wiring／authenticated preloads／intent listeners 搬移與 browser port manifest 更新。
