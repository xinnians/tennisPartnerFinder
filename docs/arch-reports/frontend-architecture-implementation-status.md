...
`frontend-architecture-fa-06-stage-4-1-manifest-gates-2026-09-07.md`。
...
| 目前批次                | `FA-06 stage 4.1` 完成：既有 manifest 新增 authenticated preload、lazy loader、unmount wiring 三個 frozen owner；lifecycle gate 不再寫死 `sessionViews.js`，且精確退役 final-v3 列出的 5 組純字面凍結。錯誤 owner canary 會紅；完整 frontend CI 通過，零 runtime 變更                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3`、`FA-06 stage 4` preflight／4.1 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                                                                        |
...
| 下一步                  | 執行 `FA-06 stage 4.2`：把 14 個 sheet dynamic imports、mount binding、preloader 與 `deferSurfaceOpen` 搬到 `src/views/surfaceLoaders.js`，同步更新 surface source owner、HTML renderer inventory 與 mutation ledger；`sessionViews` facade 與 86 個 harness calls 暫時保留。Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| FA-06 | `sessionViews` wiring、blockedPlayers、Chat／Messages ownership   | 階段 4 preflight／4.1 tests-only gate manifest 化完成；下一步為 4.2 loader 底座                                                                                                                                                                                                                                                                                        | 每個新 owner 都伴隨舊 bridge 刪除與完整回歸                                                                                                                                                               |
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   FA-06 階段 4 preflight 與 4.1 tests-only gate manifest 化已完成；下一步從 4.2
   `src/views/surfaceLoaders.js` 搬移開始。
