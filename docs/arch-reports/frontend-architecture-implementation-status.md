...
`frontend-architecture-fa-06-stage-5-1-blocked-players-facade-2026-09-07.md`。
...
| 目前批次                | `FA-06 stage 5.1` 完成：blockedPlayers 三欄與 request gate 已搬入零依賴 facade；My Sessions 舊 owner／寫入歸零，三個 refresh 入口、解除封鎖守衛、Me Page 九欄外部契約與四項競態不變量皆保留；下一步為 Stage 6 Chat／Messages 唯讀 preflight                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3`、`FA-06 stage 4` preflight／4.1～4.6 與 stage 5 preflight／5.1 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                                         |
...
| 下一步                  | 執行 `FA-06 stage 6` 唯讀 preflight：逐格重驗 Chat／Messages 的依賴與 ownership seed，先固定 unread command、Chat feed／server-state 及 Messages route bridge 的最小拆分順序。Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| FA-06 | `sessionViews` wiring、blockedPlayers、Chat／Messages ownership   | 階段 4 preflight／4.1～4.6、stage 5 preflight／5.1 完成；下一步 Stage 6 Chat／Messages 唯讀 preflight                                                                                                                                                                                                                                                                  | 每個新 owner 都伴隨舊 bridge 刪除與完整回歸                                                                                                                                                               |
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   舊 harness name 或把 blocked 三欄放回 session store。下一步做 Stage 6 Chat／Messages 唯讀 preflight，逐格重驗
   ownership seed、unread 原地變異、Chat server state 與 Messages route bridge；未完成盤點前不先決定結構。
