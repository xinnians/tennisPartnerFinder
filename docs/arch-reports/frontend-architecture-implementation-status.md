...
| 目前批次                | `FA-06 stage 6B.3` 完成：已確認 Chat 仍有 feed → controller → imperative sheet 的雙重 ownership，並固定 6B.4 React 狀態邊界、6B.5 app wiring 兩個最小退役批                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3`、`FA-06 stage 4` preflight／4.1～4.6、stage 5 preflight／5.1、stage 6 preflight／6.0／6A／6B.1／6B.2／6B.3 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                               |
...
| 下一步                  | 執行 `FA-06 stage 6B.4`：Chat feed 改成 React 可訂閱狀態，讓 loading/error/rows/archived/events/scroll 回到單一 UI owner，並實際刪除 `setState/setArchived` imperative adapter；Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   owner 與 6B.3 Chat surface 邊界 preflight 已完成；不要恢復
   原地修改 unread、Chat 直接 notify、controller 內的 feed／cursor／gate／poller owner、Messages Context hooks 或
   roster coupling，也不要把 Messages route／tab／heading 特例放回 `main.js`。下一步做 6B.4：Chat feed
   subscribe＋React UI ownership，實際刪除 `setState/setArchived` imperative adapter；完成後再做 6B.5 app wiring。
