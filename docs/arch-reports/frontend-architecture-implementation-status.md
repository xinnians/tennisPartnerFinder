...
| 目前批次                | 獨立 canonical Hosted probe 完成並復原：Google 公開 sentinel 同樣被 Cloudflare 403 擋在 Function 前，Function exact-path／limiter RPC log 0。三次不同 spoof 嘗試與官方 header 規則一致；既有 Function hash 與 DB 基線不變                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 下一步                  | 修改 source trust：canonical `cf-connecting-ip` 必填，`x-real-ip` 缺少可接受、存在時必須 canonical 且相同；同批移除 temporary probe 分支，跑完整本機 CI，再準備 no-policy Hosted canary                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| D45 | Hosted source trust 以 canonical `cf-connecting-ip` 為必要條件；`x-real-ip` 缺少可接受，存在則必須 canonical 且相同。移除短期 probe code，再用 no-policy canary 確認 source 已通過但 DB 仍為 0                                                                                                                                       | 依三次 Hosted spoof probe、C1 source substage 與 Cloudflare 官方 header 契約採用；程式與 Hosted no-policy canary 尚未完成                                                                                     |
