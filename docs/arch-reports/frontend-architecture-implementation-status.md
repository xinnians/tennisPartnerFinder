...
`frontend-architecture-production-touch-targets-2026-09-08.md`；最新 `ds-bundle` production container 校正、mobile computed-size
gate 與 WebKit select 修正在 `frontend-architecture-ds-bundle-touch-gate-2026-09-08.md`；本機 P7 閉環與剩餘外部邊界在
`frontend-architecture-ds-bundle-completion-audit-2026-09-08.md`。
最新 Function／Secret 名稱、migration、資料 aggregate 與 deployed source 唯讀重驗在
`frontend-architecture-fa-03-hosted-read-only-revalidation-2026-09-08.md`。
Cleanup C1 source substage 的 Hosted 精確結果與復原證據在
`frontend-architecture-fa-03-cleanup-c1-source-substage-result-2026-09-08.md`。
第一次 source spoof probe 的 Cloudflare 403 停點與復原證據在
`frontend-architecture-fa-03-cleanup-source-spoof-probe-2026-09-08.md`。
格式正確來源 probe 的 data-free 實作與 Hosted 精確範圍在
`frontend-architecture-fa-03-cleanup-source-canonical-probe-preflight-2026-09-08.md`。
第一次 canonical probe 的 Hosted 結果在
`frontend-architecture-fa-03-cleanup-source-canonical-probe-result-2026-09-08.md`；獨立公開 sentinel 的最小重驗範圍在
`frontend-architecture-fa-03-cleanup-source-independent-probe-preflight-2026-09-08.md`。
獨立公開 sentinel 的 Hosted 結果與來源規則依據在
`frontend-architecture-fa-03-cleanup-source-independent-probe-result-2026-09-08.md`。
最新 source trust 本機實作、完整 CI 與 no-policy Hosted canary 固定範圍在
`frontend-architecture-fa-03-cleanup-source-trust-2026-09-08.md`。
no-policy Hosted canary 的精確結果、log 欄位確認與完整復原證據在
`frontend-architecture-fa-03-cleanup-source-trust-hosted-result-2026-09-08.md`。
真正 limiter canary 重驗的固定 policy、21-request 上限、row 驗收與 guarded cleanup 在
`frontend-architecture-fa-03-cleanup-limiter-hosted-retry-preflight-2026-09-08.md`。
Hosted limiter canary 的 1 ALLOW／19 LIMIT、DB／log 證據與完整復原結果在
...
最新 Auth 登出 timeout／可取消性／可觀測性唯讀複核在
`frontend-architecture-fa-03b13-5-sign-out-timeout-preflight-2026-09-08.md`。
最新 default-off AbortSignal 接縫、真實 browser 與完整 CI 證據在
`frontend-architecture-fa-03b13-5a-abortable-sign-out-seams-2026-09-08.md`。
最新正式登出 caller／single-flight／deadline／telemetry 唯讀複核在
`frontend-architecture-fa-03b13-5b-sign-out-caller-observability-preflight-2026-09-08.md`。
最新 dormant Auth continuation core、single-flight 與完整 CI 證據在
`frontend-architecture-fa-03b13-5c-sign-out-continuation-2026-09-08.md`。
最新 default-off 正式登出外層接線、雙擊 Auth 與零 Push 副作用證據在
`frontend-architecture-fa-03b13-5d-default-off-sign-out-wiring-2026-09-08.md`。
最新 production commit、Vercel env、Hosted Function／Secret／DB 與唯一啟用順序重驗在
`frontend-architecture-fa-03b13-6-production-activation-gap-audit-2026-09-08.md`。
最新 Hosted-compatible、預設關閉的 subscription Edge source gate 與完整驗證在
`frontend-architecture-fa-03b13-6a-hosted-subscription-source-gate-2026-09-08.md`。
最新 dispatcher legacy／v2 拆分拓撲、cron 現況與 disabled worker 缺口複核在
`frontend-architecture-fa-03b13-6b-dispatcher-topology-preflight-2026-09-08.md`。
最新 dedicated、預設關閉的 v2 dispatcher source 與真實 local Edge／DB 證據在
`frontend-architecture-fa-03b13-6c-dedicated-v2-dispatcher-source-2026-09-08.md`。
最新 dispatcher DB／runtime disabled no-op、40th local migration 與零 sender／worker 證據在
`frontend-architecture-fa-03b13-6d-dispatcher-disabled-noop-2026-09-08.md`。
最新 Hosted migration／Function／Secret／cron／資料唯讀重驗與三個外部停點在
...
| 目前批次                | `FA-03B13.6g` default-off production rollout 完成：production application baseline 是 `1a3a4cb`，後續 docs-only deployment 與其 HTML／JS／CSS hash 相同；40／40 migration 與五支 Function 已部署，legacy cron 16／16 HTTP 200，三支 v2 probe 都在 hard gate 回 503，DB aggregate 不變                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| D2  | 一般登出只停止目前裝置的登入與推播；其他裝置不受影響                                                                                                                                                                                                                                                                                 | production Auth local scope 與 B13.5d default-off single-flight 已接；dormant cleanup／abortable seams 完成，但 active Push cleanup、deadline 與 server endpoint 尚未接線                                     |
...
| D7  | `ds-bundle/` 與 `.design-sync/` 保留並持續使用於 UI/UX 優化                                                                                                                                                                                                                                                                          | 全量 preflight、批次 A／B／C／D 完成；CSS／token 由 deterministic generator 同步，9 張 hand-authored 卡片有 integrity 與 mobile computed-size gate，production 44px 與 card context 缺口已修正                |
...
| D42 | C1 source substage 最小重驗只用 2 個臨時 Secret、temporary cleanup deploy、最多 2 個空 body request、零 DB write 與固定 rollback；接受兩支既有 Function version metadata 增加但 bundle hash 不變                                                                                                                                     | 使用者於 2026-09-08 明確同意；實際回 `SOURCE_REAL_MISSING`，2 requests、limiter 0、完整復原，兩支既有 bundle hash 不變                                                                                        |
| D43 | canonical source probe 只用 2 個臨時 Secret、temporary cleanup deploy、1 個已授權空 body request、公開 header sentinel、零 DB write 與固定 rollback；client output 不得包含 raw HTML／hostname／token／IP                                                                                                                            | 已完成並復原；單一 sentinel 回平台 HTML 403、Function／limiter log 0，既有 Function version 25／9→29／13 且 hash 不變；證據不足以一般化                                                                       |
...
| D48 | B13.5 不直接接正式登出：先移除 `serviceWorker.ready` 的無期限等待、把單一 AbortSignal 傳到可中止邊界，並由 shell 內部取得及重驗 current proof／binding。現有 telemetry 沒有 Push latency aggregate，所以 timeout 必須等專用 data-free canary／rollout 證據，不拿 limiter p95 代用                                                    | B13.5a default-off source seams 與真實 browser／完整 CI 已完成；沒有 timer／timeout，正式 `handleSignOut()`、runtime control 與 Hosted 仍未改                                                                 |
| D49 | 正式登出 caller 必須 single-flight；在 Auth proof 仍有效時先 best-effort Push cleanup，但 completed／pending／ignored／throw／abort 都一定繼續 current-device Auth sign-out。AbortController 由 caller 持有；timer 只能視為 best-effort deadline，不能冒充硬性 wall-clock 上限                                                       | B13.5d 已接 hard-coded disabled outer wiring；真實雙擊 Auth 1 次、cleanup／IndexedDB／SW 0，active Push 與 deadline 仍未接                                                                                    |
| D50 | legacy 與 v2 dispatcher 使用兩支獨立 scheduled Function；既有入口只處理 format 1，v2 只走 D1 commands。不得以 Hosted v2 early-return 跳過 legacy，也不把兩套 sender 綁進同一 request                                                                                                                                                 | B13.6b 唯讀複核完成；下一批先做 default-off v2 source，排程前另修 DB disabled begin 仍建立 failed worker 的缺口；Hosted cron／runtime 未動                                                                    |
| D51 | dispatcher 的 DB `dispatch_enabled=false` 或 `new_runtime_mode='disabled'` 都必須在 begin 最前端回固定 no-op；runtime 收到完整 contract 後零 claim／sender／finish，未知或不完整 contract 仍 fail closed                                                                                                                             | B13.6d local 與 B13.6f Hosted 完成；兩種 local DB branch、真實 local Edge 與 Hosted rollback transaction 都證明 worker／provider request 為 0                                                                 |
...
| FA-03 | Push runtime 與 migration                                         | preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1～B13.6f、dispatcher D0.1／D0.2／D1／D2／D3A、cleanup limiter foundation、40／40 Hosted migrations、cleanup C0、dormant C1 path、local stage diagnostic、source trust 與 limiter Hosted canary 完成；1 ALLOW／19 LIMIT；契約 v1.3；runtime disabled | expand、DB、browser-to-local-DB、雙帳號、dormant UI、default-off sign-out outer wiring＋dormant cleanup／abortable seams、dispatcher core、dedicated v2 source＋disabled no-op、local Edge、Hosted source 與 limiter canary tests 通過；active dispatcher、production cleanup Edge／policy 與不可逆 contract 另行確認 |
...
- 精確重查正式登出鏈：Me UI → profile orchestration → Auth API →
  `supabase.auth.signOut({ scope: "local" })`；Push shell 仍 hard-coded disabled，沒有 sign-out caller。
- 核對 installed `@supabase/auth-js 2.110.0`：sign-out 先等初始化與專案設定的 `-1` Auth lock，再送 logout request，
  API 沒有 sign-out AbortSignal。因此不把既有 Auth 登出冒充為已有完整 timeout。
- 依 W3C Service Workers 規格確認 `ready` 不 reject、沒有 active registration 時會繼續等待；
  `getRegistration()` 無 matching registration 時會 resolve `undefined`，適合只找既有 subscription 的登出路徑。
- 依 W3C Push API 確認 `unsubscribe()` 沒有 AbortSignal；caller 可以在 signal 後停止等待，但不能聲稱 UA 底層工作已
  取消。PostgREST RPC builder 則已確認支援 `.abortSignal(signal)`。
- IndexedDB wrapper、owner RPC、browser deactivation 目前皆未完整接 signal；B6 token cleanup fetch 已接 signal。
- current proof 可由 shell 保存的 Auth authority 取得，safe current binding 可由 runtime storage 取得；未來 shell 必須在
  await 後重驗 proof／generation／owner，不由 UI 傳入 user id。
- repo 的 Vercel Analytics 只有 `inject()`，沒有 custom `track()`；Sentry 只允許三個固定錯誤欄位，沒有 Push latency／
  outcome aggregate，不能由現有 production telemetry 推算 timeout。

本批只有文件與唯讀查證，沒有程式、測試、migration、DB、Hosted、Secret、request、deploy 或 runtime control 變更。
完整證據與固定的 `B13.5a` source-only 範圍見
`frontend-architecture-fa-03b13-5-sign-out-timeout-preflight-2026-09-08.md`。

下一步：登出 reader 改用 `getRegistration()`；owner、browser、coordinator 傳遞 signal；shell 內部取得並重驗 current
proof／binding；補 never-resolving Promise＋abort 測試。維持 disabled 零 loader／storage／SW／network，不接正式
`handleSignOut()`、不建立 timer、不填 timeout 數字。

## FA-03B13.5a default-off AbortSignal 接縫

已完成：

- 新增無 timer 的 abortable operation helper；caller abort 後可以停止等待，但不把不可取消的底層 browser operation
  冒充為已停止或已完成。
- sign-out subscription read 改用 `serviceWorker.getRegistration()`；沒有 registration 就回 absent，不 register、不問權限、
  不等 `serviceWorker.ready`。enable／refresh 的 register＋ready 流程未改。
- 新增 exact `quarantine_push_device` RPC adapter；有 signal 時使用 PostgREST `.abortSignal(signal)`。
- owner、token fallback、storage completion、browser deactivation 與 coordinator 共用 signal；abort／錯誤／shape drift 沒有
  exact completion evidence 時維持 pending／unknown，durable attempt 保留。
- production shell 新增 default-off sign-out method，由 shell 內部讀 current verified proof 與 safe binding，並在 await 後重驗
  authority／generation／owner。disabled 仍立即 ignored，零 loader／IndexedDB／SW／network。

驗證：focused Node 95／95、最後相關測試 50／50；完整 Node 716 tests（711 passed／5 skipped）、desktop＋mobile
Chromium 386 tests（382 passed／4 skipped），真實 desktop Chromium targeted 2／2。TypeScript、ESLint、Prettier、
design-system、courts seed、build 與 bundle 結構檢查通過；build 534 modules。production main
651,562／192,196／160,485 raw／gzip／Brotli；total 856,058／263,254／222,252 依 D8 report-only。重型 runtime、owner RPC、
cleanup endpoint 與 coordinator 字串命中 0；default-off shell method 命中 1。Hosted、migration、Secret、request、DB write、
runtime control 未動。程式 commit `d633441`；完整證據見
`frontend-architecture-fa-03b13-5a-abortable-sign-out-seams-2026-09-08.md`。

尚未完成：沒有正式 caller、timeout、timer、data-free aggregate metric sink、production endpoint／key／policy 或 active
runtime。下一步先做 B13.5b 唯讀接線複核；資料不足就維持 default-off，不自行填秒數。

## FA-03B13.5b 正式登出 caller／可觀測性唯讀複核

已完成：

- 現行唯一正式鏈為 MePage button → `handleSignOut()` → Auth `signOut({ scope: "local" })`；沒有 Push caller、
  pending UI、click guard 或 sign-out single-flight。
- Push cleanup 必須在 verified proof 仍有效時先 best effort 執行；但 completed／pending／ignored／throw／abort 都不能決定
  Auth 是否執行，成功／失敗 toast 仍只看 Auth 結果。
- AbortController 必須由登出 orchestration／caller 持有，因為只有 caller 負責等待時間與 Auth continuation；shell 不應決定
  產品 timeout。
- WHATWG DOM 已確認 Promise 本身不可取消，signal 只是通知；WHATWG HTML 已確認 timer 可能因 event loop 延後，未來數字只能
  稱 best-effort deadline，不是硬性 wall-clock 上限。
- `src/config.ts`、`.env.local` key 名稱、`.env.example` 與 production preview 都沒有 deadline；Vercel Analytics 無 custom
  event，Sentry 只有固定 error event。limiter p95 與 test harness timeout 都不是 sign-out cleanup 資料。

本批只有文件與唯讀查證，沒有 source、test、dependency、UI、env、migration、DB、Hosted、Secret、request、deploy 或
runtime control 變更。完整證據與 B13.5c 固定範圍見
`frontend-architecture-fa-03b13-5b-sign-out-caller-observability-preflight-2026-09-08.md`。

下一步：建立 source-only、零 timer 的 dormant Auth continuation core；注入 Push cleanup 與 Auth sign-out、接受 caller signal、
single-flight，並驗證所有 Push 結果都只執行一次 Auth。正式 `main.js`／`handleSignOut()`、telemetry 與 timeout 仍不接。

## FA-03B13.5c dormant Auth 登出 continuation core

已完成：

- 新增只接受 injected Push cleanup 與 current-device Auth sign-out 的 dormant core，沒有直接 Auth／Supabase／runtime／
  storage／network／logger／timer dependency。
- Push completed／pending／ignored／malformed、同步／非同步 throw、already-aborted 與永久 pending 後 abort 都會接著執行
  Auth；只有 Auth error 會向既有 UI 邊界傳回失敗。
- 無效 input／signal 跳過 Push並繼續 Auth，不冒險建立無法中止的等待。
- 同時間重複呼叫共用第一個 signal 與同一個 Promise，Push／Auth 都只執行一次；成功或失敗後釋放 single-flight。

驗證：continuation＋architecture／Push targeted 53／53；完整 Node 727 tests（722 passed／5 skipped）；desktop＋mobile
Chromium 386 tests（382 passed／4 skipped）。TypeScript、ESLint、Prettier、design-system、courts seed、build 與 bundle
結構檢查通過；build 534 modules。production main 651,562／192,196／160,485、total
856,058／263,254／222,252 raw／gzip／Brotli，與 B13.5a 完全相同；新 core 三個識別字與重型 runtime／RPC／endpoint
輸出命中 0。Hosted、migration、Secret、request、DB write、runtime control 未動。程式 commit `2033507`；完整證據見
`frontend-architecture-fa-03b13-5c-sign-out-continuation-2026-09-08.md`。

尚未完成：新 core 沒有 production importer、timer、deadline 或 telemetry。下一步只把它接到 hard-coded disabled shell 與
現有 `handleSignOut` 外層，讓現行登出取得 single-flight；Push 仍不得啟用。

## FA-03B13.5d default-off 正式登出外層接線

已完成：

- production main 建立 sign-out continuation，把 hard-coded disabled shell 與既有 `handleSignOut` 注入；Me UI 的登出改走
  continuation，同一筆 in-flight 只會執行一次 Push shell＋Auth。
- shell disabled 時立即 ignored，不載入 runtime；既有 Auth local scope、成功／失敗 toast 與私人 UI 清理行為未改。
- integration unit 驗兩次呼叫共用 Promise、Auth 1 次、runtime loader 0；source gate 固定 continuation 唯一 importer 是 main，
  且 main 仍有 exact disabled shell。
- 真實 local Supabase＋Chromium 在同一 task 雙擊：logout POST exact 1、cleanup request 0，Push IndexedDB 與 Service Worker
  registration 前後皆 0；登出後私人 UI 清空、既有成功 toast 正常。

驗證：targeted 54／54、真實 local sign-out 1／1；完整 Node 729 tests（724 passed／5 skipped）、desktop＋mobile mock
Chromium 386 tests（382 passed／4 skipped）。TypeScript、ESLint、Prettier、design-system、courts seed、build 與 bundle
結構檢查通過；build 535 modules。production main 653,283／192,738／160,678、total
857,779／263,789／222,464 raw／gzip／Brotli；main gzip 超既有觀察值 318 bytes、total raw／gzip 超 7,818／4,727，依 D8
開發期 report-only。重型 runtime、RPC、cleanup endpoint 命中 0；小型 outer core 命中 1。Hosted、migration、Secret、request、
runtime control 未動。程式 commit `7d1280c`，browser 證據 commit `d147f22`；完整報告見
`frontend-architecture-fa-03b13-5d-default-off-sign-out-wiring-2026-09-08.md`。

尚未完成：active Push cleanup、deadline／AbortController owner、telemetry、production config 與 Hosted runtime。下一步先做
B13.6 activation gap 唯讀稽核，不直接開 gate 或填數字。

## FA-03B13.6 production activation gap 稽核

已完成：

- Vercel metadata 證明 `qiuka.tw` production 是 commit `a14e81e`；它是目前工作分支 `7110782` 的 ancestor，
  `git rev-list` 實算相差 124 commits。部署 commit 的 tree 只有 legacy Push，這批 default-off v2 程式尚未上線。
- Vercel production env 只讀確認有 legacy VAPID public key 名稱，沒有兩套 v2 public JWK、mode 或 deadline 名稱；未讀 value。
- Supabase Hosted 重新確認只有 active dispatcher version 41 與獨立 canary version 25；subscription／cleanup Function 不存在。
  canary 10 個 source dependency 仍和 repo 逐 byte 相同；active dispatcher `index.ts` 仍是 legacy deployed source，repo 的
  format-1 compatible filter／local-only v2 route 尚未部署。
- Hosted migration 39／39；DB exact aggregate 維持 runtime disabled、4 legacy／0 v2 Push、7 v1／0 pending outbox，
  worker／canary profile／delivery／consent／registry／limiter 全為 0，所有時間／attempt policy 仍未配置。
- 已把 source-only、default-off deploy、config、canary evidence、小比例 rollout、legacy cutoff 排成唯一順序，並逐項標出
  deploy／Secret／request／runtime write 等外部停點。

本批只有文件與只讀外部 metadata／aggregate／source download；沒有 deploy、Secret mutation、Function request、DB write 或
production web request。完整證據見
`frontend-architecture-fa-03b13-6-production-activation-gap-audit-2026-09-08.md`。

下一步：做 B13.6a，只補 `push-subscription-v2` Hosted-compatible、預設關閉的 source gate 與測試；沒有 exact opt-in 或完整
config 時必須維持 503。
...
- runtime gate 新增 exact `hosted-v1`；只有 Hosted marker 與該 mode 同時成立才是 `hostedEnabled`，local mode 與 Hosted mode
  不可互用，尾端空白也不接受。
- 新 `entrypoint.js` 集中 gate、Origin、handler 與 adapters；disabled、Origin 不合法或任何建構錯誤都回相同 data-free 503，
  且沒有 logging。`index.ts` 現在只讀 env 並呼叫單一 entrypoint。
- Hosted path 明確把 `localTestEnabled=false` 傳給 adapter，本機 HTTP 例外不會被帶進正式環境；既有 local mode 與真實
  browser → Edge → Auth → DB → IndexedDB 測試保持通過。

驗證：targeted Node 28／28、真實 local Edge 1／1；完整 Node 730 tests（725 passed／5 skipped）、mock Chromium 386 tests
（382 passed／4 skipped）。TypeScript、ESLint、Prettier、courts seed、design-system、build 與 bundle structure 通過；build
535 modules，bundle 與 B13.5d 完全相同。程式 commit `34062b3`；完整證據見
`frontend-architecture-fa-03b13-6a-hosted-subscription-source-gate-2026-09-08.md`。

本批沒有 migration、Hosted deploy、Secret、Function request、DB write、Vercel env／deploy 或 runtime control 變更。Hosted
目前仍沒有 subscription Function／Secret，DB mode 仍 disabled。

下一步：先做 B13.6b active dispatcher entry 唯讀複核，不直接部署，也不填尚無正式量測依據的時間與次數設定。

## FA-03B13.6b dispatcher topology 唯讀複核

已完成：

- 核對 Hosted 目前 4 個 active cron；只有既有 legacy dispatcher 每分鐘執行，沒有 v2 dispatcher cron。
- 核對 legacy entry 是 REST／snapshot／attempts 流程；D1／D3A v2 是單一 DB transaction、generation barrier 與 deadline
  流程。直接用 Hosted v2 分支提早 return 會跳過 legacy；把兩套流程塞進同一 request 則會綁住 timeout、錯誤與 rollback。
- 固定採兩支 scheduled Function：保留 `notification-outbox-dispatch` 只處理 format 1，新增
  `notification-outbox-dispatch-v2` 只經 D1 commands 處理 format 2，兩者可獨立停用與回復。
- 查出一項啟用前缺口：DB mode disabled 時，現行 begin command 仍可能建立 worker，claim 才回 disabled，runtime 最後把它記成
  failed。dedicated v2 source 可先 default-off 落地，但排程前必須另批修成 disabled no-op。
- 固定 v2 namespaced config、legacy／v2 分段 deploy 與 rollback 順序；本批沒有填任何 policy 數字。

本批只有文件、repo source 與 Hosted cron／Function／DB 唯讀查證；沒有 source、migration、deploy、Secret、Function request、DB
write 或 runtime control。完整依據見
`frontend-architecture-fa-03b13-6b-dispatcher-topology-preflight-2026-09-08.md`。

下一步：做 B13.6c source-only dedicated v2 Function；維持預設 unavailable 且不新增 schedule。B13.6d 再以 additive migration／
runtime test 修 disabled no-op，兩批皆不直接啟用 Hosted。

## FA-03B13.6c dedicated v2 dispatcher source

已完成：

- 新增獨立 `notification-outbox-dispatch-v2`，不再讓 v2 與 legacy 共用 scheduled entry；沒有新增 cron。
- Hosted 只接受 exact `hosted-v1`，local 只接受 exact `local-test-v1`；mode／DB URL／generation／batch／bounded cron secret
  缺一就固定 503，授權失敗前 sender／DB 都不執行。
- v2 DB、generation、batch、policy、transport 與 local mock URL 全改用 namespaced config；Hosted 永遠不能選 mock，只共享既有
  VAPID 三個名稱。
- entry 只組合既有 D1 database port、D3A Deno-native sender、local test sender 與 runtime core，沒有複製 claim／send／
  outcome 邏輯，也沒有 logging。
- 真實 local Edge → 專用 DB role → held transaction → mock provider 成功；format 2 完成，format 1 的 attempts／sent_at 不變。

驗證：dedicated unit 8／8、真實 local Edge 2／2；完整 Node 738 tests（733 passed／5 skipped）、mock Chromium 386 tests
（382 passed／4 skipped）。TypeScript、ESLint、Prettier、courts seed、design-system、build、bundle structure 與 diff check
通過；build 535 modules，bundle bytes 與 B13.6a 相同。程式 commit `25cd2ea`；完整證據見
`frontend-architecture-fa-03b13-6c-dedicated-v2-dispatcher-source-2026-09-08.md`。

本批沒有 migration、Hosted deploy／Secret／request／DB write／runtime control、Vercel 變更或 production policy 數字。現有 Hosted
cron secret 只確認名稱存在，沒有讀 value／長度，因此未宣稱它已符合新 entry 的 32～256 bytes 要求。

下一步：做 B13.6d additive DB／runtime disabled no-op；先在 local 驗證，不建立或啟用 Hosted cron。

## FA-03B13.6d dispatcher disabled no-op

已完成：

- 新增 additive migration `202609080001_notification_dispatcher_disabled_noop.sql`；只替換既有 worker begin body，保留
  signature、owner、`security definer`、空 `search_path`、revoke 與專用 role grant。
- `dispatch_enabled=false` 或 `new_runtime_mode='disabled'` 時，在 generation／policy check 與 worker insert 前回固定 disabled
  contract；兩種分支都不建立 worker。
- runtime 只接受兩個 exact disabled code 與完整 canonical contract，立即回 `{kind:"disabled",version:1}`；缺欄位或未知 code
  仍 fail closed。
- dedicated entrypoint 延後到第一筆真正 send 才建立 sender；停用時零 claim／sender／finish。
- 真實 local Edge 先驗 disabled 200、worker 0、mock provider request 0，再切 local canary fixture 驗既有 transaction／send；
  legacy format 1 row 保持不變。

驗證：40 migrations 從零重播；targeted pgTAP 56／56、完整 DB 18 files／1,294 tests、DB lint 0 error、targeted Node
20／20、真實 local Edge 2／2。完整 frontend CI 為 Node 741 tests（736 passed／5 skipped）、mock Chromium 386 tests
（382 passed／4 skipped），build 535 modules，bundle bytes 不變。程式 commit `da44bab`；完整證據見
`frontend-architecture-fa-03b13-6d-dispatcher-disabled-noop-2026-09-08.md`。

本批只在 local 套第 40 支 migration。2026-09-08 linked list 實查為 40 local／39 Hosted，dry-run 只列這一支 pending；沒有
Hosted apply、Function deploy、Secret／cron／request／DB write／runtime control 或 production policy 變更。

下一步：先做外部停點的唯讀 preflight；migration apply、legacy compatible deploy 與 disabled v2 deploy 必須分批回報並驗收，
不能一次推全部 production 變更。

## FA-03B13.6e Hosted 分段部署 preflight

已完成：

- linked migration list 與 dry-run 實查 40 local／39 Hosted，唯一 pending 是 `202609080001`，沒有 seed／role 變更。
- Hosted Function 實查只有 legacy version 41 與 canary version 25；正式 v2、subscription v2、cleanup 都不存在。
- 只讀 17 個 Secret 名稱：既有 cron／VAPID 與 canary 名稱存在，正式 v2 與 `WEB_PUSH_TRANSPORT` 不存在；沒有讀 value。
- DB aggregate 實查 runtime disabled、policy null、workers／deliveries 0、Push 4 legacy／0 v2、outbox 7 format 1／0 format 2，
  pending 全 0；4 個 cron 只有一個指向 legacy，v2 cron 0。
- 重新下載 legacy version 41 source：`dispatch.js` 與 repo byte-identical；`index.ts` 差異精確是尚未部署的 format-1 filter、
  Hosted mock 禁止、local-only v2 route 與排版。repo 的 v2 runtime 另多 B13.6d no-op；Hosted legacy path 不會呼叫 v2 batch。
- 最近 5 筆 cron job run 都 succeeded，但明確不把 pg_cron enqueue 成功冒充 Function HTTP 200；停點 A 另查 exact-path log。
- 固定 M 只套 migration、A 只更新 legacy、B 只部署 disabled v2；三步不能合併。

驗證：legacy／format source targeted Node 16／16；repo clean 基準 `abcb623`。完整 evidence、驗收與 rollback 見
`frontend-architecture-fa-03b13-6e-hosted-staged-deploy-preflight-2026-09-08.md`。
...
- 依使用者持續 migration 授權，套用 dry-run 唯一列出的 `202609080001_notification_dispatcher_disabled_noop.sql`；
  沒有 seed／role 或其他 migration。
- 套用後 40 local／40 Hosted，dry-run up to date；begin function 的 local／Hosted definition MD5 都是
  `69490509d40c9f8dc3bde24a70217f7f`。
- catalog 確認兩個 disabled guard 都在 worker insert 前；owner／security definer／空 search path 不變，execute ACL 只有
  `postgres` 與 `notification_dispatcher`。
- Hosted transaction 呼叫回 exact `runtime_mode_disabled`、同 transaction worker 0，最後 rollback；persistent aggregate
  也維持 worker 0。
- runtime control、Push 4 legacy／0 v2、outbox 7 format 1／0 format 2、pending、delivery、cron 都維持套用前基線。
- Function 仍是 legacy version 41／canary 25；17 個 Secret 名稱不變，正式 v2 名稱仍不存在。

驗證：linked lint 0 error；strict pg-delta 12,326 bytes、15 default privilege、51 revoke、51 grant、0 drop／other，和 D1
Hosted apply 的既有 platform ACL baseline 完全相同。完整證據與並行 Management API 查詢停滯／逐一重跑紀錄見
`frontend-architecture-fa-03b13-6f-hosted-disabled-noop-migration-2026-09-08.md`。
...
- 依使用者明確要求加速並推送全部安全變更，production application baseline `1a3a4cb` 已推到 `origin/main`；Vercel
  application deployment `dpl_BbdDasmg61cgydN1wmwTi2ZgJoiw` 為 `READY`，驗收時 `qiuka.tw` 已指向它。
- 後續 docs-only deployment 也已 `READY`，且 HTML／JS／CSS SHA-256 和 application deployment 完全相同；文件 commit
  沒有被誤算成新的 application baseline。
...
## `ds-bundle` 批次 A deterministic CSS 同步

- 新增 `scripts/designSystemBundle.mjs`：直接從 `src/main.js` 讀 production CSS imports，依真實 cascade order 產生
  `_ds_bundle.css`；同時從唯一含 `--color-ink` 的 production `:root` 產生 token reference。輸出無 timestamp，
  同 source 可重現相同 bytes。
- 新增 `npm run sync:design-system` 與 `npm run check:design-system`；checker 已接進 pretest、完整 unit aggregate 與
  `test:ci:frontend`。正式 workflow 原本就呼叫該 aggregate，因此沒有修改 workflow。
- 新增 3 個 gate tests：production entrypoint 與 manifest 順序一致、兩個 committed artifacts byte-exact、修改
  `30dvh` canary 後必須變紅。`tests/ci-config.test.js` 另鎖 commands 與實際執行入口。
- `_ds_bundle.css` 現由 13 份 production CSS 產生，semantic audit 為 668 identical、0 changed、0 bundle-only、
  0 production-only；token 仍是 50 個且零 drift。
- README／config／conventions 已改為品牌「球咖」與 React 現況，只索引 repo 確實存在的 9 張卡；NOTES 保留舊歷史
  並明確標成歷史。本批沒有遠端 read-back，所以沒有宣稱遠端內容已變。
- 更新後 9 張卡在 desktop／390px 共 18/18 渲染，console／page／request error、overlay、overflow 都是 0；
  BottomNav 互動通過。既有 viewport／Toast ID／小型 demo control 問題留到批次 B。
- 完整 frontend CI 通過：Node 670／5 skipped、mock Chromium 348／4 skipped、build 與 bundle structural checks 通過。
  production bundle bytes 未變，development total 依 D8 仍只報 raw／gzip 超過 3,599／3,348。
- 詳細證據見 `frontend-architecture-ds-bundle-deterministic-sync-2026-09-08.md`。本批沒有修改 production UI／CSS、
  migration、Hosted、secret、deployment 或資料。

## `ds-bundle` 批次 B 卡片校正

- 逐張對照現行 production component／view owner 更新 9 張 HTML 卡片；三張缺少的 viewport meta 已補齊，Toast
  重複 `map-data-status` ID 已改為單一 error 態示範。
- Chat 補齊檢舉／封鎖／取消參加、空名冊、空訊息、封存提示與停用 composer；BottomNav／topbar 改記目前 React
  owner；Sheet 補上現行「練球」、關閉 selector 與 shell／content 分工。
- 42 個不重複 production source 引用都解析到現存檔案；舊 `.js` owner、固定行號、歷史 commit／批次說法與不存在
  的 Screens 引用已清除。
- `check:design-system` 新增 9-card explicit inventory、viewport、stylesheet、unique ID、source citation 與 retired
  screen gate；缺 viewport、重複 ID、失效 source 三種 canary 都會變紅。targeted 36／36。
- static audit 共 175 classes；兩個 Chat 動態 variant 與一個沒有獨立 CSS 的 FilterSheet 語意 class 都由現行 source
  證實，不當成 orphan。
- desktop 1280×900＋mobile 390×844 共 18／18：HTTP、viewport、console／page／request、ID、accessible name、
  form label、overflow 與 BottomNav 互動全部通過；代表性 screenshots 已人工查看。
- computed-size 報告仍有 desktop 47／mobile 25 個展示控制項低於 44px。部分已由 390px production test 證明是
  card context 假陽性，部分基底仍確實是 34／36／38／40px；下一批先在真實 route／container 量測分類，不直接猜修法。
- 完整 frontend CI 通過：Node 672／5 skipped、mock Chromium 348／4 skipped、build／bundle checks 通過；production
  bundle 未變。
...
- Supabase 官方 logging 文件與 C0 都證明 platform logs 會保存 `cf-connecting-ip` 與 `x-real-ip`；DB 只存 HMAC
  digest 無法去除平台層 raw IP。使用者已確認方案為 Free 並接受 1 天 retention；B13.3 已在 privacy 頁揭露
  Supabase Edge 短期來源 IP 與未來 v2 IndexedDB 範圍。正式開放時仍須逐項核對實際設定沒有超出文案。
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   `npm run sync:design-system`。批次 D 與 completion audit 已完成：Buttons／Chips 有真正的 production container，
   mobile computed-size 已納入 card gate 且有 43px negative canary；不要再改已經 0 違規的 production 規則，
   也不要把 mock marker 或未 read-back 的遠端設計專案當正式結果。
...
4. `FA-03B5`～`FA-03B9` 已建立 dormant storage、bounded cleanup transport、owner RPC adapter、single-attempt
   ...
   B13.5 已完成 timeout／可觀測性唯讀盤點，B13.5a 已以 `getRegistration()`、單一 AbortSignal、exact owner RPC adapter
   與 shell current proof／binding 重驗完成 default-off 接縫；但正式 `handleSignOut()`、timeout、endpoint 與 active runtime
   尚未接。B13.5b 已確認 caller 必須 single-flight、Push 所有結果都要繼續 Auth、timer 只可稱 best-effort deadline；
...
| 2026-09-08 | FA-03 Hosted 唯讀重驗            | 兩支 Hosted Function／17 個 Secret 名稱、39／39 migration、runtime／資料 aggregate、cron／專用 role 與 deployed source 全部重查。canary 10 files byte-identical；legacy `index.ts` 差異精確符合已知未部署 D2／D3A source，無未知 drift；Hosted 零寫入、零 Function request。                                                                                                                  |
| 2026-09-08 | cleanup C1 source substage       | 使用者核可 2 個臨時 Secret／temporary deploy／最多 2 requests／零 DB write 後執行。未授權 503 exact RETRY；授權精確回 `SOURCE_REAL_MISSING`。outer log 仍見兩來源欄位，limiter RPC／row 0；Function／Secret 完整移除，既有 bundle hash 不變。                                                                                                                                                 |
| 2026-09-08 | cleanup source spoof probe       | client 確實送出無效 `cf-connecting-ip`，Cloudflare 在 Function 前回 403／error 1000；同窗 Function exact path log 0。只能證明無效格式無法穿透，不外推到 canonical 假 IP。rollback 全 0，cleanup 資源 0，DB 基線不變；既有 Function version metadata 各 +4、bundle hash 不變。                                                                                                                 |
| 2026-09-08 | cleanup canonical probe          | exact canary 授權後新增短期 data-free compare，response 只含三種固定 stage；body／key／limiter／quarantine call 全 0，35 tests 與 lint／format 通過。Hosted 範圍固定為 2 個臨時 Secret、1 deploy、1 request、零 DB write與完整 rollback；尚未執行外部操作。                                                                                                                                   |
| 2026-09-08 | cleanup canonical result         | client 確認送出格式正確的 reserved／probe headers；平台回 HTML 403，Function exact path／limiter RPC unified log 皆 0。rollback 全 0，cleanup 資源 0，DB 基線不變；既有 Function version 25／9→29／13、hash 不變。單一 sentinel 不外推為一般結論，下一批做獨立公開 sentinel。                                                                                                                 |
| 2026-09-08 | cleanup independent probe        | 第二個 sentinel 已由 Google 官方確認為公開 DNS IP，只作 header 比較、不連線；固定沿用 2 個臨時 Secret、1 deploy、1 request、零 DB write與 mode → Function → token rollback。程式仍是已通過 35 tests 的 data-free probe；Hosted 尚未執行。                                                                                                                                                     |
| 2026-09-08 | cleanup independent result       | Google 公開 sentinel 同樣被 Cloudflare 403 擋在 Function 前；server marker／`cf-ray` present，Function exact path／limiter RPC log 0。rollback 全 0，cleanup 資源 0，DB 基線不變；既有 Function version 29／13→33／17、hash 不變。三次 probe 與官方 header 契約支持 D45。                                                                                                                     |
| 2026-09-08 | cleanup source trust             | `cf-connecting-ip` 維持 canonical 必填，`x-real-ip` 缺少可接受、存在則驗格式與一致；temporary probe 與 `SOURCE_REAL_MISSING` 已移除。targeted 34／34、frontend Node 679／5、Chromium 378／4、DB 1,290／1,290 與完整 local／preview／Edge CI 通過；Hosted no-policy canary 尚未執行。                                                                                                          |
| 2026-09-08 | cleanup source Hosted result     | 單一已授權空 request exact 503／RETRY／`POLICY`，證明 source 通過、無 policy 時 fail closed；Function exact-path log 1、limiter RPC／row 0。mode／Function／token rollback 皆 0，既有 Function 33／17→37／21 且 hash 不變，Secret 17、Push／outbox／runtime 基線不變。                                                                                                                        |
...
| 2026-09-08 | FA-03B13.5 preflight             | 唯讀確認正式 Auth sign-out 無 signal、Auth lock `-1`，Push 的 IndexedDB／owner RPC／`serviceWorker.ready`／unsubscribe 仍有等待邊界；`getRegistration()` 與 PostgREST `.abortSignal()` 可移除部分結構阻塞。現有 Vercel／Sentry 無 Push latency aggregate，不猜 timeout；下一批只做 default-off abortable seams。                                                                              |
| 2026-09-08 | FA-03B13.5a                      | 完成 source-only、default-off abortable seams：sign-out read 改 `getRegistration()`，owner／browser／coordinator／shell 共用 signal，shell 自取並重驗 current proof／binding。Node 716 tests（711／5）、Chromium 386 tests（382／4）、真實 targeted 2／2、build 534 modules；程式 commit `d633441`。正式 `handleSignOut()`、timer／timeout、Hosted／migration／runtime control 未動。         |
| 2026-09-08 | FA-03B13.5b preflight            | 唯讀確認現行登出無 click guard／single-flight；Push 必須在 Auth 前 best effort，但所有結果都要繼續 Auth。AbortController 由 caller 持有；repo 無 deadline／Push aggregate，且 timer 不是硬性 wall-clock 保證，因此不猜秒數。下一批只做 dormant、零 timer 的 Auth continuation core。                                                                                                          |
| 2026-09-08 | FA-03B13.5c                      | 完成 dormant Auth continuation：Push 所有結果／throw／永久 pending 後 abort 都繼續 Auth；併發呼叫共用第一筆 Push＋Auth，Auth error 後可重試。targeted 53／53、Node 727 tests（722／5）、Chromium 386 tests（382／4）、build 534 modules；bundle 與 B13.5a 相同且新 core 命中 0。程式 commit `2033507`，production／timer／Hosted 未接。                                                       |
| 2026-09-08 | FA-03B13.5d                      | production Me 登出已接 hard-coded disabled continuation single-flight；真實同步雙擊只有 1 次 Auth logout，cleanup request／Push IndexedDB／SW 前後皆 0。targeted 54／54、local 1／1、Node 729 tests（724／5）、mock Chromium 386（382／4）、build 535；重型 runtime 命中 0。程式 `7d1280c`、browser test `d147f22`，timer／timeout／Hosted 未接。                                             |
| 2026-09-08 | FA-03B13.6                       | 重驗 Vercel／Supabase Hosted：`qiuka.tw` 是 `a14e81e`，比工作分支少 124 commits且只有 legacy Push；Hosted Function 只有 dispatcher 41／canary 25，subscription／cleanup 0，DB mode disabled、v2 row 0、39／39 migration。固定六段啟用順序與外部停點；只有文件及唯讀 metadata／aggregate／source download，未 deploy／改 Secret／發 request／寫 DB。                                           |
| 2026-09-08 | FA-03B13.6a                      | subscription Edge 新增 Hosted-compatible、default-off entrypoint：Hosted marker＋exact `hosted-v1` 才建立 handler，其餘與建構錯誤固定 503；local mode 不變。targeted 28／28、真實 local Edge 1／1、Node 730 tests（725／5）、Chromium 386（382／4）、build 535；程式 `34062b3`，未 deploy／改 Secret／發 request／寫 DB。                                                                     |
| 2026-09-08 | FA-03B13.6b                      | 唯讀確認 legacy／v2 必須拆成兩支 scheduled Function；現有 4 個 cron 只有 legacy dispatcher，v2 schedule 為 0。另查出 DB disabled 時 begin 仍可能建立 failed worker，固定在排程前另批修成 no-op。本批只有文件與 source／Hosted metadata 查證，未改 source、Hosted、Secret、request、DB 或 runtime。                                                                                            |
...
| 2026-09-08 | FA-03B13.6g                      | 依使用者加速要求完成 default-off production rollout：application baseline `1a3a4cb`、Vercel READY，後續 docs-only deployment 的 HTML／JS／CSS hash 相同；40／40 migration、五支 Function deployed 且逐檔符合 repo。legacy cron 16／16 HTTP 200；三支 v2 probe exact 503，runtime/policy/DB data 不變。desktop/mobile 正式站載入與篩選互動通過；active keys／cron／UI／runtime 仍關閉。 |
