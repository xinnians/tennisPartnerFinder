...
結構 gate 證據在 `frontend-architecture-fa-04-phase-0a-structure-gates-2026-09-07.md`；最新 phase 0b ownership 基線在
`frontend-architecture-fa-04-phase-0b-ownership-baseline-2026-09-07.md`。
...
| 目前批次                | `FA-04 phase 0b` 已完成：正式 DOM mutation 基線為 17 files／125 AST nodes／34 symbols／112 API-target refs；browser ports 分為 controller direct 1、platform adapters 26、UI globals 54、type-only 152；四個 React 外長駐 live roots 在三種 browser project 經資料更新與頁面切換後仍保持同一 DOM identity。完整 frontend CI 為 Node 638 tests／633 passed／5 skipped、Playwright 354 tests／350 passed／4 skipped；只改測試、manifest 與文件，沒有 runtime／UI／Hosted 變更                                                                                                                                                                                                                                    |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b` 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                                                                                                                                |
...
| 下一步                  | 進入 `FA-05`：先做 final-v3 階段 1 已證明的低風險清理與 before／after 驗證，再處理 production preview、效能基線與 Bundle ADR。Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認；runtime policy、generation／legacy cutoff 再分批                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   FA-04 phase 0a／0b 已完成，可先獨立進行 `FA-05` 已核可的低風險清理。
