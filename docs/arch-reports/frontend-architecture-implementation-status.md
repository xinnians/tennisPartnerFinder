...
| 目前批次                | `FA-03B13.5` Auth 登出 timeout／可取消性唯讀複核完成：確認 `serviceWorker.ready`、IndexedDB、owner RPC、browser unsubscribe 與 Auth sign-out 的實際等待邊界；現有 telemetry 無 Push latency aggregate，因此沒有猜 timeout。正式 `handleSignOut()`、endpoint、timeout 與 runtime 仍未接線                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell／B13.3 dormant UI mapping＋privacy／B13.4 dormant sign-out cleanup／B13.5 timeout preflight、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3`、`FA-06 stage 4` preflight／4.1～4.6、stage 5 preflight／5.1、stage 6 preflight／6.0／6A／6B.1～6B.5 已完成；第一個 vertical slice 完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                               |
...
| 下一步                  | 做 `FA-03B13.5a` source-only abortable seams：登出 reader 改用 `getRegistration()`、owner／browser／coordinator 傳遞 signal、shell 內部取得 current proof＋binding；disabled 必須零副作用。不接正式 `handleSignOut()`、不建立 timer、不猜 production endpoint／key／policy／timeout                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
