...
| 目前批次                | 第一次 canonical Hosted probe 完成並復原：client headers exact，但平台回 HTML 403、Function exact-path／limiter RPC log 都是 0。只證明該單一 sentinel 沒進 Function，不外推其他 canonical IP；既有 Function hash 與 DB 基線不變                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| 下一步                  | 改用 Google 官方確認的公開 canonical sentinel 做 1 次獨立 Hosted probe；仍是 2 個臨時 Secret、1 deploy、1 request、零 DB write與完整 rollback。在第二個結果完成前不改 source trust                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| D44 | 用 Google 官方確認的 `8.8.8.8` 作第二個獨立 header sentinel；沿用同一 data-free code 與 D43 exact scope，不向該 IP 連線、不增加 request 或 DB write                                                                                                                                                                                  | 使用者已委託依進度文件持續執行，並在得知 temporary Hosted probe／rollback 後回覆繼續；尚未執行                                                                                                                |
