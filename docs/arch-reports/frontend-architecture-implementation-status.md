...
| 目前批次                | cleanup source spoof probe 第一步完成：client 自訂無效 `cf-connecting-ip` 被 Cloudflare 403／error 1000 擋在 Function 前；Function log 0、零 DB write、完整 rollback。這只證明無效格式無法穿透，尚未涵蓋格式正確的假 IP                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| 下一步                  | 新增短期 data-free canary 比較，讓 Function 只回「格式正確的 client probe 是否等於收到的 `cf-connecting-ip`」固定 stage；local 驗證後 temporary deploy、只送 1 request、零 DB write並復原。在得到結果前不改 source trust                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
