...
`frontend-architecture-fa-05-phase-1-low-risk-cleanup-2026-09-07.md`。
...
| 目前批次                | `FA-05 phase 1` 已完成：移除 1 個開機時 `authSession` 必為 null 的 no-op preload，只保留 verified identity transition 的有效 preload；`SurfaceSlot`、`PROFILE_PUBLIC_DISCLOSURE`、`sessionFormSheetRuntime` 改為檔內私有。完整 frontend CI 為 Node 639 tests／634 passed／5 skipped、Playwright 354 tests／350 passed／4 skipped；production JS 減少 21 raw／32 gzip bytes                                                                                                                                                                                                                                                                                                                                                      |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1` 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                                                                                                               |
...
| 下一步                  | 繼續 `FA-05`：先唯讀盤點 production-equivalent preview、效能基線與 Bundle ADR 所需的現有 Vite／Vercel／Playwright 條件，再建立不需要 production Secret 的可重現量測。Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認；runtime policy、generation／legacy cutoff 再分批                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| FA-05 | 低風險清理、production preview、效能基線、Bundle ADR              | phase 1 低風險清理完成；preview／效能／ADR 待做                                                                                                                                                                                                                                                                                                                        | before／after 可重現；未放寬未核可邊界                                                                                                                                                                    |
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   FA-04 phase 0a／0b 與 FA-05 phase 1 低風險清理已完成；下一步先做 FA-05 production preview／效能／Bundle ADR
   的唯讀 preflight。
...
| 2026-09-07 | FA-05 phase 1                    | 刪除開機時 auth null 的 no-op preload，只留 verified identity transition 的有效 preload；三個零外部 import 的名稱取消 export 並加 source gate。Node 639／5 skipped、Playwright 350／4 skipped、build／bundle gates 通過；production JS -21 raw／-32 gzip bytes。                                              |
