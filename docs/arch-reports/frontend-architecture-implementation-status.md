...
| 目前批次                | `FA-03B13.5a` source-only、default-off AbortSignal 接縫完成：登出 reader 已改用 `getRegistration()`；owner RPC、browser deactivation、coordinator 與 shell 共用 signal；shell 自行讀取並重驗 current proof／binding。沒有 timer 或 timeout 數字，正式 `handleSignOut()`、endpoint、active runtime 仍未接線                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell／B13.3 dormant UI mapping＋privacy／B13.4 dormant sign-out cleanup／B13.5 timeout preflight／B13.5a abortable seams、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3`、`FA-06 stage 4` preflight／4.1～4.6、stage 5 preflight／5.1、stage 6 preflight／6.0／6A／6B.1～6B.5 已完成；第一個 vertical slice 完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                   |
...
| 下一步                  | 做 `FA-03B13.5b` 正式 caller／observability 唯讀複核：固定 AbortController owner、Auth sign-out 必定繼續的控制流，以及 production deadline 的可驗證資料來源。資料不存在就維持 default-off，不接正式 `handleSignOut()`、不新增 metric sink、不猜 production endpoint／key／policy／timeout                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| FA-03 | Push runtime 與 migration                                         | preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1～B13.5a、dispatcher D0.1／D0.2／D1／D2／D3A、cleanup limiter foundation、39／39 Hosted migrations、cleanup C0、dormant C1 path、local stage diagnostic、source trust 與 limiter Hosted canary 完成；1 ALLOW／19 LIMIT；契約 v1.3；runtime disabled | expand、DB、browser-to-local-DB、雙帳號、dormant UI、dormant sign-out cleanup＋abortable seams、dispatcher core、local Edge、Hosted source 與 limiter canary tests 通過；active dispatcher、production cleanup Edge／policy 與不可逆 contract 另行確認 |
...
4. `FA-03B5`～`FA-03B9` 已建立 dormant storage、bounded cleanup transport、owner RPC adapter、single-attempt
   ...
   尚未接。下一批先做 caller／data-free observability 唯讀複核；不得為了接線自行猜數字。server-only origins 不得進 browser，也不得
