...
`frontend-architecture-ds-bundle-deterministic-sync-2026-09-08.md`。
...
| 目前批次                | `ds-bundle` 批次 A 完成：13 份 production CSS／50 tokens 已由 deterministic generator 同步，drift gate 已接 pretest、unit aggregate 與 frontend CI；下一批逐張更新 9 張卡片                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| bundle checker／CI 變更 | production checker 已分開 development report／release enforce 並報 raw／gzip／Brotli；design-system checker 另鎖 13 份 CSS 順序、產生檔與 50 tokens。required CI 跑兩個 checker 與 production preview Chromium；WebKit preview 留在非阻擋 job                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 下一步                  | `ds-bundle` 批次 B：逐張對照現行 component／view owner 更新 9 張卡片，補 viewport meta、修 Toast 重複 ID並重跑 desktop／390px；Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| D7  | `ds-bundle/` 與 `.design-sync/` 保留並持續使用於 UI/UX 優化                                                                                                                                                                                                                                                                          | 全量 preflight 與批次 A 完成；CSS／token 改由 deterministic generator 與 CI gate 同步，9 張 hand-authored 卡片尚待批次 B 更新                                                                                 |
...
- `ds-bundle` 的 CSS／token mirror 與 drift gate 已完成；9 張 hand-authored 卡片仍有舊 owner／citation、三張缺
  viewport meta、Toast 重複 ID 與部分 demo control 小於 44px。在批次 B 完成前，只能信任產生 CSS，不能把卡片
  DOM／狀態當成 production 正典或直接回同步。
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   比對。`ds-bundle` 批次 A deterministic CSS／token generator、drift gate 與品牌／React／索引文件同步也已完成；
   不要手改 `_ds_bundle.css` 或 `tokens/tokens.css`，production CSS 變更後要跑 `npm run sync:design-system`。
   下一步做批次 B：逐張對照現行 component／view owner 更新 9 張卡片、補三張 viewport meta、修 Toast 重複 ID，
   再跑 source／markup audit 與 desktop／390px render；不先改 production UI。
