...
| 目前批次                | `FA-05 phase 2` 已完成：新增 production build＋preview 的未登入、已登入、OAuth PKCE callback、390px Chromium 慢網路與 WebKit 延遲基線；同一個 production marker probe 有 anonymous negative 與 authenticated positive control；bundle checker 新增 Brotli 報告但不設門檻。production-equivalent before／after main 與 total raw／gzip／Brotli 差異皆為 0；完整 frontend、local Supabase 與 preview 測試通過                                                                                                                                                                                                                                                                                                                                                         |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2` 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                                                                                                           |
...
| 下一步                  | 繼續 `FA-05 phase 3`：只做 Bundle ADR 與可丟棄 PoC 設計，比較 facade、完整 client dynamic load、subpackage、調整 gate 與直接 composition；本階段不加 dependency、不改 byte gate、不加 alias。Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認；runtime policy、generation／legacy cutoff 再分批                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| FA-05 | 低風險清理、production preview、效能基線、Bundle ADR              | phase 1／2 完成；Bundle ADR 待做                                                                                                                                                                                                                                                                                                                                       | before／after 可重現；未放寬未核可邊界                                                                                                                                                                    |
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   FA-04 phase 0a／0b 與 FA-05 phase 1／2 已完成；production preview／效能基線不要重做。下一步只做 FA-05
   phase 3 Bundle ADR 與可丟棄 PoC 設計，不加 dependency、不改 byte gate、不加 alias。
