...
| 目前批次                | `FA-03B13.5d` default-off 正式登出外層接線完成：Me UI 已經過 continuation single-flight，再走既有 `handleSignOut()`；shell 仍 hard-coded disabled，真實雙擊只送 1 次 Auth logout，cleanup request／Push IndexedDB／SW 都是 0。沒有 timer／timeout 或 telemetry，Push v2 仍未啟用                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| 下一步                  | 做 `FA-03B13.6` production activation gap 唯讀稽核：把 config、Hosted source、server gate、browser caller、deadline 與 rollout evidence 的實際缺口排成唯一順序，區分可繼續的 source-only 工作與需要另停下確認的 deploy／Secret／runtime control／request。不改 production mode、不猜 endpoint／key／policy／timeout                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
