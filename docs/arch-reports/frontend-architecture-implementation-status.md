...
| 目前批次                | `FA-03B12.9` browser → real local Edge → Auth／DB → IndexedDB composition 已完成；下一批先做 production wiring preflight                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、cleanup limiter foundation 與 Hosted additive migration 已完成；Hosted runtime 尚未啟用                                                                                                                                                                                                                                                                                                                                                             |
...
| 下一步                  | 只讀盤點 production Auth adapter、runtime config、UI／SW importer、B1／B9 handoff 與 server-controlled gate；不填 production provider／key／timeout 值、不解除 Hosted gate                                                                                                                                                                                                                                                                                                                                                                                                      |
...
4. `FA-03B5`～`FA-03B9` 已建立 dormant storage、bounded cleanup transport、owner RPC adapter、single-attempt
   ...
   composition，B12.9 已驗證 browser → encryption → Edge → Auth → A4 DB RPC → local CAS。下一步先做 production
   wiring preflight，只讀盤點 Auth adapter、runtime config、UI／SW importer、B1／B9 handoff 與 server-controlled gate；
   server-only origins 不得進 browser，也不得自行猜 production provider／key。cleanup hosted 完成前不得接
