...
| 目前批次                | `FA-05 phase 1` 已完成：移除 1 個 auth-null no-op preload，只保留 verified identity transition 的有效 preload；3 個檔內名稱改私有；移除 13 個零 production caller 的 presentation runtime re-export 與 10 組失效 F2D／Prettier 鷹架，保留仍有 consumer 的 2 個 helper re-export。完整 frontend CI 為 Node 639 tests／634 passed／5 skipped、Playwright 354 tests／350 passed／4 skipped；production JS 較 FA-04B 減少 21 raw／32 gzip bytes                                                                                                                                                                                                                                                       |
