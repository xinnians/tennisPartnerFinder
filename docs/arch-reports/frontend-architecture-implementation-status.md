...
`frontend-architecture-fa-06-stage-5-blocked-players-preflight-2026-09-07.md`。
...
| 目前批次                | `FA-06 stage 5` 唯讀 preflight 完成：確認 blockedPlayers 恰有 3 個 production refresh 入口、controller 3 欄、My Sessions 3 組寫入、auth identity 的 5-key reset、解除封鎖同步守衛與 Me Page 9-key 契約；零依賴 facade 的最小原子範圍及四項競態不變量已固定                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3`、`FA-06 stage 4` preflight／4.1～4.6 與 stage 5 preflight 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                                     |
...
| 下一步                  | 執行 `FA-06 stage 5.1`：建立零依賴 blockedPlayers facade，原子搬移三欄狀態與 request gate，接回三個 refresh 入口、auth account clear、解除封鎖守衛與 Me Page 訂閱，再跑完整回歸。Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| FA-06 | `sessionViews` wiring、blockedPlayers、Chat／Messages ownership   | 階段 4 preflight／4.1～4.6 完成；stage 5 blockedPlayers preflight 完成，下一步 5.1 facade 實作                                                                                                                                                                                                                                                                        | 每個新 owner 都伴隨舊 bridge 刪除與完整回歸                                                                                                                                                               |
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   4.6 facade 完整退役已完成；不要恢復 `sessionViews.js` 或舊 harness name。下一步做 stage 5 blockedPlayers
   唯讀 preflight，先重查三個 refresh 入口、controller／auth reset／Me Page 契約與四項競態不變量。
