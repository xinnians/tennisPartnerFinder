...
| 下一步                  | 做 `FA-03B13.5c` source-only dormant Auth continuation core：注入 Push cleanup＋Auth sign-out、接受 caller signal、single-flight，並驗證 Push completed／pending／ignored／throw／malformed／永久 pending 後 abort 都只執行一次 Auth。不改 `main.js`／`handleSignOut()`、不建立 timer／metric sink、不猜 endpoint／key／policy／timeout                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| D49 | 正式登出 caller 必須 single-flight；在 Auth proof 仍有效時先 best-effort Push cleanup，但 completed／pending／ignored／throw／abort 都一定繼續 current-device Auth sign-out。AbortController 由 caller 持有；timer 只能視為 best-effort deadline，不能冒充硬性 wall-clock 上限 | B13.5b 唯讀確認現行 UI 無 click guard、repo 無 deadline／aggregate；下一批只做 dormant continuation core，不接 UI／timer／telemetry |
...
| FA-03 | Push runtime 與 migration                                         | preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1～B13.5b、dispatcher D0.1／D0.2／D1／D2／D3A、cleanup limiter foundation、39／39 Hosted migrations、cleanup C0、dormant C1 path、local stage diagnostic、source trust 與 limiter Hosted canary 完成；1 ALLOW／19 LIMIT；契約 v1.3；runtime disabled | expand、DB、browser-to-local-DB、雙帳號、dormant UI、dormant sign-out cleanup＋abortable seams／caller preflight、dispatcher core、local Edge、Hosted source 與 limiter canary tests 通過；active dispatcher、production cleanup Edge／policy 與不可逆 contract 另行確認 |
...
4. `FA-03B5`～`FA-03B9` 已建立 dormant storage、bounded cleanup transport、owner RPC adapter、single-attempt
   ...
   尚未接。B13.5b 已確認 caller 必須 single-flight、Push 所有結果都要繼續 Auth、timer 只可稱 best-effort deadline；下一批
   只做 dormant continuation core，不得為了接線自行猜數字。server-only origins 不得進 browser，也不得
