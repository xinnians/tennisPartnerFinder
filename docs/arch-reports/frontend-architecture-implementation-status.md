...
| 目前批次                | Hosted source trust 本機實作完成：`cf-connecting-ip` 必填，`x-real-ip` 缺少可接受、存在時必須 canonical 且相同；temporary source probe 已移除。targeted 34／34、完整 frontend／Supabase CI 通過；Hosted no-policy canary 尚未執行                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 下一步                  | 先將 source trust 本機批次建立獨立 commit，再依固定範圍執行 1 次 no-policy Hosted canary；預期 exact 503／RETRY／`POLICY`，limiter 與其他 mutable DB 維持 0，最後完整復原                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| D45 | Hosted source trust 以 canonical `cf-connecting-ip` 為必要條件；`x-real-ip` 缺少可接受，存在則必須 canonical 且相同。移除短期 probe code，再用 no-policy canary 確認 source 已通過但 DB 仍為 0                                                                                                                                       | 本機程式、temporary probe 移除、targeted 34／34 與完整 frontend／Supabase CI 已完成；Hosted no-policy canary 尚未執行                                                                                         |
