...
| 目前批次                | cleanup C1 source substage Hosted 重驗完成：未授權 request 正常；授權 request 精確回 `SOURCE_REAL_MISSING`，證明 Function 內缺 `x-real-ip`。2 requests、零 DB write、完整 rollback；既有 Function bundle hash 不變                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 下一步                  | 以現有 hard-gated Function 做一次 client-supplied invalid `cf-connecting-ip` spoof-resistance diagnostic；在證明平台會覆寫前不改 source trust。仍只用臨時 2 Secret／temporary deploy、1 個授權 request、零 DB write、固定 rollback                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
