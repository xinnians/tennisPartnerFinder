...
| 目前批次                | canonical source probe 本機完成：exact canary 授權後只比較兩個 canonical IP 並回固定 stage，body／key／limiter／quarantine call 全為 0；35 tests 與 lint／format 通過。Hosted 尚未 deploy 或 request                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| 下一步                  | 先獨立 commit 本機 probe；再唯讀確認 Hosted 基線，temporary deploy、只送 1 個 canonical probe request、確認零 DB write並完整復原。在得到精確結果前不改 source trust                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| D43 | canonical source probe 只用 2 個臨時 Secret、temporary cleanup deploy、1 個已授權空 body request、公開 header sentinel、零 DB write 與固定 rollback；client output 不得包含 raw HTML／hostname／token／IP                                                                                                                            | 使用者在已說明 temporary Hosted probe 與完整 rollback 後於 2026-09-08 回覆繼續；本機 data-free 分支與 35 tests 已完成，Hosted 尚未執行                                                                        |
