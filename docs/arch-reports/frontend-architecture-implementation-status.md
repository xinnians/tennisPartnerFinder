...
| 目前批次                | `FA-03B13.2` default-off composition shell 已完成；`FA-03B13.3` UI／狀態／隱私需先確認產品文案與切換範圍                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| 下一步                  | 使用者先確認 `FA-03B13.3` 四種 Push 狀態文案、UI／隱私與 production config 的分批方式、legacy 切換時機；cleanup Hosted 與 dispatcher barrier 完成前不接 v2 真實 request                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| FA-03 | Push runtime 與 migration                                         | preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、cleanup limiter foundation、Hosted additive migration、cleanup C0、dormant C1 path、local stage diagnostic 與 source substage 完成；Hosted diagnostic 停在 `SOURCE` 且已復原；契約 v1.3；runtime disabled | expand、DB、browser-to-local-DB、雙帳號測試通過；UI／privacy、dispatcher、source substage Hosted 重驗、production Edge 與不可逆 contract 另行確認 |
...
4. `FA-03B5`～`FA-03B9` 已建立 dormant storage、bounded cleanup transport、owner RPC adapter、single-attempt
   ...
   shell，重型 runtime 未進 production 產物。下一步 B13.3 先確認四種狀態文案、UI／privacy 與 config 分批、legacy
   切換時機。server-only origins 不得進 browser，也不得自行猜 production provider／key。cleanup Hosted 與 dispatcher
