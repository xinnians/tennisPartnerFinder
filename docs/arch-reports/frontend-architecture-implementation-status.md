...
| 目前批次                | dispatcher D3B 已把 DB URL／generation／provider／transport 改成 canary 專用 Secret 名稱並通過完整 CI；Hosted 仍只有 D1 migration，尚未設定 credential／Secret 或部署 canary。B13.3 UI 另等待產品決策                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 下一步                  | 用專用 role 實際驗證平台 DB connection，設定六個 canary-only Secret 並只部署獨立 Function；不送 request、不改 runtime／cron。真正 browser fixture、runtime policy、canary request、generation／legacy cutoff 再分批；B13.3 UI 另等待產品確認                                                                                                                                                                                                                                                                                                                                                         |
