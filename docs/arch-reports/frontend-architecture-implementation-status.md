...
| 下一步                  | 做 `FA-03B13.5d` default-off production outer wiring：`main.js` 注入 disabled shell＋現有 `handleSignOut`，Me UI 改走 continuation，取得 single-flight；驗證零 runtime load／IndexedDB／SW／network 且 Auth／toast 行為不變。不建立 timer／metric sink、不改 shell disabled、不猜 endpoint／key／policy／timeout                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| D2  | 一般登出只停止目前裝置的登入與推播；其他裝置不受影響                                                                                                                                                                                                                                                                                 | `FA-03B10.1` 已固定 production Auth local scope；B10.2／B13.4／B13.5a 已完成 dormant cleanup 順序、`getRegistration()` 與 abortable seams，正式 `handleSignOut()`、timeout 與 server endpoint 尚未接線                 |
...
| D49 | 正式登出 caller 必須 single-flight；在 Auth proof 仍有效時先 best-effort Push cleanup，但 completed／pending／ignored／throw／abort 都一定繼續 current-device Auth sign-out。AbortController 由 caller 持有；timer 只能視為 best-effort deadline，不能冒充硬性 wall-clock 上限 | B13.5c dormant continuation core 已完成；727 Node／386 Chromium 與零 production importer 已驗證，下一批只接 hard-coded disabled outer wiring |
...
| FA-03 | Push runtime 與 migration                                         | preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1～B13.5c、dispatcher D0.1／D0.2／D1／D2／D3A、cleanup limiter foundation、39／39 Hosted migrations、cleanup C0、dormant C1 path、local stage diagnostic、source trust 與 limiter Hosted canary 完成；1 ALLOW／19 LIMIT；契約 v1.3；runtime disabled | expand、DB、browser-to-local-DB、雙帳號、dormant UI、dormant sign-out cleanup＋abortable seams／continuation core、dispatcher core、local Edge、Hosted source 與 limiter canary tests 通過；active dispatcher、production cleanup Edge／policy 與不可逆 contract 另行確認 |
...
4. `FA-03B5`～`FA-03B9` 已建立 dormant storage、bounded cleanup transport、owner RPC adapter、single-attempt
   ...
   B13.5c dormant continuation core 已完成並保持 production 零 importer。下一批只接 disabled outer wiring，不得為了接線
   自行猜數字。server-only origins 不得進 browser，也不得
