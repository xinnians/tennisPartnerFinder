...
`frontend-architecture-fa-06-stage-4-wiring-preflight-2026-09-07.md`。
...
| 目前批次                | `FA-06 stage 4` 唯讀盤點完成：已核對 81 行／3 consumers 的既有 manifest、14 lazy sheets、14 unmount registrations、4 configure、2 preload listeners、86 個 `sessionViews` harness calls，以及 final-v3 所指 5 組可退役字面 gate 的歷史原文。下一批先做 tests-only 的 gate manifest 化，不改 runtime                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3`、`FA-06 stage 4` preflight 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                                                                             |
...
| 下一步                  | 執行 `FA-06 stage 4.1`：在既有 `surfaceManifest.js` 加入結構 owner 路徑，讓 lifecycle gate 不再寫死 `sessionViews.js`，並精確退役 final-v3 列出的 5 組純字面凍結；先通過新 gate 再開始搬 loader。Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認；runtime policy、generation／legacy cutoff 再分批                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| FA-06 | `sessionViews` wiring、blockedPlayers、Chat／Messages ownership   | 階段 4 唯讀 preflight 完成；下一步為 4.1 tests-only gate manifest 化                                                                                                                                                                                                                                                                                                   | 每個新 owner 都伴隨舊 bridge 刪除與完整回歸                                                                                                                                                               |
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   FA-06 階段 4 唯讀 preflight 已完成；下一步從 4.1 tests-only gate manifest 化開始。
