...
`frontend-architecture-fa-06-stage-6a-chat-feed-owner-2026-09-07.md`。
...
| 目前批次                | `FA-06 stage 6A` 完成：Chat feed facade 已接管 messages、roster、read cursor、request gate 與 poller；controller 舊 owner 已刪除，下一步 6B Messages route owner                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3`、`FA-06 stage 4` preflight／4.1～4.6、stage 5 preflight／5.1、stage 6 preflight／6.0／6A 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                   |
| runtime 變更            | Auth gate、current-device local sign-out、DB dormant command、本機 cleanup Edge、兩套獨立 public-key build boundary、dormant IndexedDB／cleanup transport／owner adapter／coordinator／Auth handoff／Push deactivation／manual re-enable coordinator／pre-network cancel／refresh commit／subscription browser／transport／local composition、dormant Push v2 validator／hybrid crypto／Edge ports、local-only Push v2 HTTP／Auth／DB handler、default-off app composition shell、dormant dispatcher egress／outcome core、local D1 DB barrier、local-only D2 Edge／DB／mock、D3A Deno-native sender、獨立 Hosted canary no-write probe、local-only cleanup limiter composition，以及 Chat feed owner 已落地；Push 登出 server cleanup、v2 UI、SW、Hosted active dispatcher 尚未接線 |
...
| 下一步                  | 執行 `FA-06 stage 6B`：Messages 專用 selector＋page-route owner，集中 deep link、click、back/forward 與 heading focus，並同批刪除 `main.js` 特例與 App services bridge；Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| FA-06 | `sessionViews` wiring、blockedPlayers、Chat／Messages ownership   | 階段 4 preflight／4.1～4.6、stage 5 preflight／5.1、Stage 6 preflight／6.0／6A 完成；下一步 6B Messages route owner                                                                                                                                                                                                                                                    | 每個新 owner 都伴隨舊 bridge 刪除與完整回歸                                                                                                                                                               |
...
- 最新一般 development build 的 main 為 650,916／191,738／160,027，total 為
  853,571／261,909／221,057 raw／gzip／Brotli；raw／gzip 比目前參考值多 3,610／2,847 bytes。D8 允許開發期
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   unread command 與 6A Chat feed owner 已完成；不要恢復原地修改 unread、Chat 直接 notify，或 controller 內的
   feed／cursor／gate／poller owner。下一步做 6B Messages 專用 selector＋page-route owner，同批刪除 `main.js`
   Messages 特例與 App services bridge。
