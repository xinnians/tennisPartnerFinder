...
證據在 `frontend-architecture-fa-06-stage-6-0-unread-command-2026-09-07.md`。
...
| 目前批次                | `FA-06 stage 6.0` 完成：My Sessions owner 提供 immutable `clearMySessionUnread()` command，Chat 已刪除原地 mutation 與直接 notify 依賴；下一步 6A Chat feed owner                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3`、`FA-06 stage 4` preflight／4.1～4.6、stage 5 preflight／5.1、stage 6 preflight／6.0 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                      |
...
| 下一步                  | 執行 `FA-06 stage 6A`：建立 Chat feed facade，接管 messages、roster、read cursor、request gate 與 poller，並同批刪除 controller 內被取代的 owner；Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| FA-06 | `sessionViews` wiring、blockedPlayers、Chat／Messages ownership   | 階段 4 preflight／4.1～4.6、stage 5 preflight／5.1、Stage 6 preflight／6.0 完成；下一步 6A Chat feed owner                                                                                                                                                                                                                                                             | 每個新 owner 都伴隨舊 bridge 刪除與完整回歸                                                                                                                                                               |
...
- 最新一般 development build 的 main 為 650,708／191,652／159,939，total 為
  853,363／261,848／220,969 raw／gzip／Brotli；raw／gzip 比目前參考值多 3,402／2,786 bytes。D8 允許開發期
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   舊 harness name 或把 blocked 三欄放回 session store。Stage 6 Chat／Messages 唯讀 preflight 與 6.0 immutable
   unread command 已完成；不要恢復原地修改 `unreadMessageCount` 或 Chat 的直接 notify 依賴。下一步做 6A Chat
   feed owner，接管 messages、roster、read cursor、request gate 與 poller，並同批刪除 controller 內舊 owner。
