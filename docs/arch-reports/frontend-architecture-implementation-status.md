...
`frontend-architecture-fa-05-phase-3-bundle-adr-2026-09-07.md`。
...
| 目前批次                | `FA-05 phase 3` Bundle ADR 已完成：採 A 維持完整 Supabase client＋現有 facade；B dynamic full client、C production alias、D 調 gate 現階段不採；E 只在正式效能證據成立時做可丟棄 App-level PoC。已核對 9 個 production named imports、boot 即需 Auth＋PostgREST、目前套件 attribution、PostgREST 2.110.0 沒有 `setAuth()`，以及現行 14＋8 個 non-byte／Sentry provenance call sites。本批只有文件                                                                                                                                                                                                                                                                                                                                                                   |
| 整體狀態                | `FA-00`、`FA-01`、`FA-02` 完成；FA-03 preflight、`FA-03A0`～`FA-03A4`、`FA-03B0`～`FA-03B11.1`、`FA-03B12.1`～`FA-03B12.9`、`FA-03B13` preflight／B13.1 Auth adapter／B13.2 disabled shell、dispatcher D0.1／D0.2／D1／D2／D3A local composition、cleanup limiter foundation、全部 39 份 Hosted migration 與獨立 Hosted canary 執行環境、`FA-04 phase 0a／0b`、`FA-05 phase 1／2／3` 已完成；active D2／D3A source 尚未部署 Hosted，Hosted v2 runtime 未啟用                                                                                                                                                                                                                                                                                                        |
...
| 下一步                  | 進入 final-v3 階段 4／本表 `FA-06`：先唯讀重查現有 `surfaceManifest.js`、lazy／wiring／可及性 gates 與 86 個 harness calls，再規劃 gate-first 的最小 wiring 批；不重開已裁決的 Bundle E PoC。Hosted C 真實 Push 與 B13.3 UI 仍分別等待產品確認；runtime policy、generation／legacy cutoff 再分批                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
1. 確認分支為 `codex/frontend-architecture-execution`，先讀本文件、`FA-03B10` Push v2 contract v1.3、
   ...
   下一步從 final-v3 階段 4／本表 FA-06 的 gate-first 唯讀 preflight 開始。
