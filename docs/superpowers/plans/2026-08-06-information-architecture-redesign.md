...
| 9a 新使用者預設訂全市 | **暫緩**(2026-08-07,ian:「批 9 皆暫緩」)→**已併入**(2026-08-17,ian 拍板 cherry-pick;新 hash `4b22346`+`9302309`,測試 selector 對齊 v2 checkbox 網格 `f2ec68a`;read-back 六整合點 PASS,local 40 綠/mock 246 綠) | 工作已完成並**停放於分支 `claude/batch-9a-court-subscription-seed`**(`f7f9e0b` 種入 + `418c00a` NTRP 夾帶項) | 實作方在 ian 拍板前先做完前端版,PM 驗收 gate 全綠(`db 774 / unit 211 / mock 177+3 / local 37+8 / mobile 4 / build 67`,migrations 與 `dataApi.js` 零 diff)。**未併入工作分支**。恢復時直接 cherry-pick 該分支兩個 commit。<br>**關鍵設計(比 PM 原構想好,值得保留)**:判斷訊號不是「訂了幾座」(那正是分不清的東西),而是**存檔前資料庫有沒有這個人的 profiles 列**——因為 `private.ensure_notification_profile()` 被 7 個通知 RPC 呼叫,先訂球場再建檔的人也會有列,故不會被重種。<br>**殘餘缺口**:建檔當下球場清單載入失敗則永遠不補(實作方自曝)。<br>**PM 更正**:實作方回報種入 55 座,實際 **53**(台北 active 53／新北 29／inactive 3);55 是把測試 diff 的行數當項目數。<br>**未解**:一次無法重現的紅(`session.spec.js:999`,該輪耗時 2 分鐘 vs 平常 55 秒,之後完整套件 7 次全綠、單獨 10 次全綠;推測資源競爭,**無錯誤原文故未證實**)。**有時效**:只對新使用者在「首次完成個人檔案」時生效,發布期沒做的話那批人永遠零訂閱,且不可事後隱式補寫(資料模型分不出「沒選過」與「選了零座」)。純前端零 migration。已驗證錨點:`main.js:362` `const saved = await saveCurrentProfile(draft);`、`main.js:730` `await saveCourtSubscriptions(nextCourtIds);`、`dataApi.js:782`／`:830` 定義、`sessionController.js:924` `state.courts`、上限見 `202608050001:11-18`。**落點由實作方提案後才動工** |
...
| 8 定詞表+視覺+a11y 收尾 | **ACCEPTED**(0 blocker) | `ea4a41b` | 六個程式 commit(`38e8c2b..ea4a41b`;PM 的 docs commit 夾在中間,與程式無交集)。「對拉」清查 PM 代查完(hosted 單打 9／雙打 3／**對拉 1**,`EDIT_PLAY_TYPES` 不可收窄,既有測試綠)。**三處偏離派工單,全部是 PM 錯**:①A2 用詞——PM 寫「主揪婉拒了你的申請」會撞掉**兩條**既有守衛(`smoke.spec.js:1377`、`session.spec.js:837` 的 `not.toContainText("主揪婉拒")`),且違反 SPEC:179 定詞表(統一詞是「婉拒」,未要求點名主揪),更與批 7 已上線 hosted 的推播 body 打架;實作方改用「你的加入申請已被婉拒」與推播**逐字一致**。②PM 建議在 `sessionViews.js:1594` 補 `esc()` 會造成**雙重轉義**(PM 實測 `&amp;amp;`),該值已在 `:1616` 邊界轉義。③觸控缺口**實測 13 個非 10 個**(建局 8、編輯 5),PM 的 10 是手數的繼承值。**PM 完全漏掉的兩項**:自己的聊天泡泡 `#e5f0ff` 對比 **4.0903**(最差,且非 mist 底,PM 的「mist 系容器」框法掃不到)、圖釘內點 `fill` 也是 `#99aac1`(**2.3664**,PM 只點名描邊)。 |**建局/編輯表單既有 44px 缺口經獨立重量為 7+3=10 個(非 37),約 6 條 CSS**;`.surface__close` 全站 sheet 共用,獨立 commit 且守衛用對稱式 |
...
1. **新使用者預設訂全台北市球場 = 要做**。理由:D6 指出冷啟動期球場廣播是唯一把新供給主動
   推給需求的管道;而訂閱只是偏好,**真正收到通知還需使用者另外授權瀏覽器推播**
   (派送端需裝置 endpoint,見 `dispatch.js:28-32`),所以預設訂全市不等於騷擾。
   批 6 的 F4 把 0 訂閱改成預設收合,解的是頁長,**沒有解「怎麼讓新使用者訂全市」**。
   - [已驗證] 訂全市在 DB 層合法:`202608050001_court_subscription_limit.sql:11-18` 的
     `max_court_count` 即台北市 active 球場總數,判斷式是 `> max_court_count` 才拒絕,
     等於總數是允許的。**不需要 migration**。
   - **歸屬批 9**(與共用選擇器同屬球場選擇域,且皆為純前端)。
   - 設計要點:資料模型無法區分「從未選過」與「明確選了零座」(兩者都是零列)。
     因此不可在「我」頁做隱式補寫,只可在**首次完成個人檔案**這個前端已知的時點種入。
     實作前先提案落點。
2. **共用球場選擇器 = 開完整批 9,三個呼叫點共用一個元件**。
   訂閱(0-53、有全選)/ 常打球場(1-∞、無全選)/ 候選球場(恰好 2-3、無全選),
   含分組或搜尋 + min/max + 即時計數。
   驗收方的支持理由:常打球場是 directory gate 必填(進球友目錄的必經關卡),
   轉換率影響大於訂閱設定;候選球場的「恰好 2-3 座」約束目前只有一句 hint、無即時計數。
   **硬約束**:另兩處**絕不可**套用批 6 那套「全選」語意——「常打全台北 53 座」是荒謬宣告
   且會餵進 `player_directory` 給別人看;候選球場放「全選」等於放一顆違反 DB 約束的按鈕。
   共用的是元件與互動,不是 min/max 與全選的預設值。
3. **批 7 的 hosted migration 時機 = 驗收後立刻套用,再繼續 push 分支**。
   理由:[推論] `src/dataApi.js` 的查詢是明列欄位,前端一旦帶上 `played_count` 而 hosted DB
   沒有該欄,PostgREST 會讓整個查詢失敗——球友目錄與加入前名單會直接壞掉。
   順序硬性為 **migration 先、前端後**。套用時走 `docs/mvp-plan.md` 的 release checklist
   (備份/count preflight/`npx supabase migration list` 對齊/匿名 REST allowlist 複驗)。
4. **種子供給劇本 = 等批 8 做完再說**,現在不並行起草。
...
12. **不變式一旦建立,後續批次的驗收條件必須帶著它**。
    **目前已建立的不變式清單(每批派工單逐條檢查是否適用,不要只帶一半)**:
    - **I1 觸控目標**:390px 下「我」頁全部互動元素 ≥44×44;守衛在
      `tests/session-mobile.spec.js` 用對稱掃描 + 數量下限,新增控件要讓下限同步上調。
    - **I2 焦點對照表**:凡在 `renderMePage` 新增或搬入可 tab 控件,
      `captureMeFocus` 與 `resolveMeFocus` **必須成對**加分支;
      只加一邊不會 compile error,只會在重繪後靜默把焦點丟到 heading/body。
    - **I3 三頁互斥**:`showMapPage` / `showMySessionsPage` / `showMePage` 任一時刻只有一個非 hidden。
    - **I4 匿名邊界**:`session_discovery` 的主揪欄位不得擴充。
    - **I5 反向斷言配正向前提**:`toHaveCount(0)` / `doesNotMatch` 一律要有證明掃描集非空的正向斷言。
    (批 4 的 blocker 正是 I2 漏帶:派工單只寫了 I1 的「新增入口要落在對稱守衛掃描範圍內」,
    沒提焦點表,結果不變式建立後的第一個新控件就漏登記。)批 3a 建立「我」頁 44px 觸控不變式,
