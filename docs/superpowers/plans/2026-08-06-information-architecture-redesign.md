...
| 9 共用球場選擇器 + 新使用者預設訂全市 | **暫緩**(2026-08-07,ian:「先這樣使用看看」) | — | 原拍板為開全範圍,同日改為暫緩,等真實使用行為再定方向。三個呼叫點共用元件,但 min/max 與「全選」語意**各自不同**;純前端零 migration。詳見下方「ian 拍板」1、2。**重啟訊號**:使用者實際抱怨挑球場麻煩,或 directory gate(常打球場必填)被觀察到卡住轉換 |
