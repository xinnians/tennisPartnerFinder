...
## 待 ian 拍板(2026-08-07,批 7 前)

1. **要不要預設幫新使用者訂全台北市球場?** PM 傾向要。理由:D6 指出冷啟動期球場廣播是唯一
   把新供給主動推給需求的管道;而訂閱只是偏好,**真正收到通知還需使用者另外授權瀏覽器推播**
   (派送端需裝置 endpoint,見 `dispatch.js:28-32`),所以預設訂全市不等於騷擾。
   批 6 的 F4 把 0 訂閱改成預設收合,解的是頁長,**沒有解「怎麼讓新使用者訂全市」**。
2. **共用球場選擇器要不要開批 9?** 服務三個呼叫點(訂閱 0-53 有全選 / 常打球場 1-∞ 無全選 /
   候選球場恰好 2-3 無全選),含分組或搜尋 + min/max + 即時計數。
   PM 傾向**先不做**,等真實使用者抱怨再定方向。
   驗收方的反對意見值得記:常打球場是 directory gate 必填(進球友目錄的必經關卡),
   轉換率影響大於訂閱設定;候選球場的「恰好 2-3 座」約束只有一句 hint、無即時計數。
   **注意**:另兩處**絕不可**套用批 6 這套兩段式——「常打全台北 53 座」是荒謬宣告且會餵進
   `player_directory` 給別人看;候選球場放「全選」等於放一顆違反 DB 約束的按鈕。
