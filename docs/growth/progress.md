...
最後更新：2026-09-09。G02／G03／G04／G10 已上線；G09 已擴至 10 篇並加入名稱／行政區查找，均已正式部署，全台 562 筆待審底稿與品質報告已建立。正式站驗證及真實成效分開記錄。

## 當前狀態

- 使用者已核可依競品清單開始改善、建立目標與維護跨 session 進度。
- 最新範圍：G09 十篇球場指南與名稱／行政區查找、全台待審底稿，以及已上線的 G10 逐局分享預覽；本批必要回歸通過，十篇已於 qiuka.tw 上線。G01、G05–G08、G11–G12 暫緩。
...
| G09 | 擴充已核可；十篇查核完成 | 完成；全台底稿待審 | 通過，已知 Safari 時間限制保留 | 十篇已部署，底稿未公開 | 未取得 |
...
- 十五篇內容完成；新增百齡社子岸、觀山、成美右岸、民權及台北網球場。775 unit／384 Chromium、4 local API／46 local browser、20 production-preview 通過；PR #5 待必要 CI 與部署驗證。查核差異與未完成範圍見 [第二批紀錄](g09-guide-batch-two-2026-09-09.md)。

## 下一步

1. 指南查找已上線；下一輪可依品質報告審核其餘台北與新北資料；不要把 562 筆自動發布或用來停用既有球場。依使用者實際回饋修正 G09／G10；若要取得轉換／留存數據，先重新確認是否啟動已暫緩的 G01，不把工程測試當產品成效。
