...
最後更新：2026-09-10。G02／G03／G04／G10 已上線；G09 已擴至 28 篇並加入名稱／行政區查找，均已正式部署，全台 562 筆待審底稿與品質報告已建立。正式站驗證及真實成效分開記錄。

## 2026-09-10 待確認指南公開（實作／驗證中，正式仍28篇）

使用者核可未知欄位明示後公開資料頁，取代先前必須資格／現況全明才可發布的規則。本機新增33篇，總61篇；23篇開放狀態待確認且無指南開局／訂閱／近期球局，38篇保留動作。位置有疑義者不提供導航；沒有改DB場地狀態。URL、登入pending intent及controller皆限制受確認狀態控制的指南開局。來源排除tennislocal／jojotennis，原始來源日期保留。

[本批規格與QA](court-guide-pending-publication-2026-09-10.md)已保存。5項指南單元通過；完整frontend與test:local進行中，未部署，正式仍28篇。下一步完成內容／桌面與390px／no-JS／入口驗證、必要CI及Git發布，產品成效未取得。
...
## 2026-09-10 未發布33筆第二輪補證（最新，研究進行中）
