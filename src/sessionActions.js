...
export function setMySessionActionScope(root, scopeKey) {
...
function currentMySessionActionButton(root, descriptor) {
  ...
  return [...root.querySelectorAll("[data-my-action]")].find((button) =>
    sameActionDescriptor(actionDescriptor(button), descriptor)
  );
}

export function syncPendingMySessionActions(root) {
...
export function runMySessionAction(button, callback, root) {
...
export async function runNotificationSettingAction(root, callback) {
...
export async function runPresenceSettingAction(root, callback) {
