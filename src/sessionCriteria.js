function finiteCoordinate(value) {
  if (value == null || value === "") return null;
  const number = Number(value);
  return Number.isFinite(number) ? number : null;
}

function latitude(point) {
  return finiteCoordinate(point?.courtLat ?? point?.lat);
}

function longitude(point) {
  return finiteCoordinate(point?.courtLng ?? point?.lng);
}

export function isUndecidedCandidate(session) {
  return String(session?.venueType) === "candidates" && !Boolean(session?.decidedAt);
}

export function isSessionFull(session) {
  if (String(session?.status).toLowerCase() === "full") return true;
  return session?.slotsRemaining != null && Number(session.slotsRemaining) <= 0;
}

export function isJoinableSession(session) {
  return String(session?.status).toLowerCase() === "open" && !isSessionFull(session);
}

export function distanceMeters(left, right) {
  const lat1 = latitude(left);
  const lng1 = longitude(left);
  const lat2 = latitude(right);
  const lng2 = longitude(right);
  if ([lat1, lng1, lat2, lng2].some((value) => value == null)) return Number.POSITIVE_INFINITY;
  const radians = Math.PI / 180;
  const latitudeDelta = (lat2 - lat1) * radians;
  const longitudeDelta = (lng2 - lng1) * radians;
  const a =
    Math.sin(latitudeDelta / 2) ** 2 +
    Math.cos(lat1 * radians) * Math.cos(lat2 * radians) * Math.sin(longitudeDelta / 2) ** 2;
  return 6_371_000 * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
}
