...
// 既有 6 座球場名稱：跨後端的 join key(profile 的球場勾選以「名稱」比對),
// 絕不可從資料檔中消失,否則既有 profile/partner_requests 會斷鏈。
const LEGACY_SIX = [
  "台北網球中心",
  "大安森林公園網球場",
  "中正網球中心",
...
function validateCourts(courts) {
  ...
  // 1. name 唯一非空;含「臺」即報錯(顯示名稱一律「台」);既有 6 座名稱逐字存在
  ...
  for (const legacyName of LEGACY_SIX) {
    if (!nameCounts.has(legacyName)) {
      errors.push(`既有 6 座名稱缺漏,不可從資料檔移除(join key):${legacyName}`);
    }
  }
 ...
 * 雛形階段尚無新北資料時,改用任一非大安樣本(有真實新北資料後自動切換)。
 */
function pickSample(courts) {
  const newTaipei = courts.find((c) => c.city === "新北市");
  if (newTaipei) return newTaipei.name;
  const nonDaan = courts.find((c) => c.name !== "大安森林公園網球場");
  return nonDaan ? nonDaan.name : courts[0].name;
}
...
function buildMigration(courts) {
  ...
  const valuesLines = courts
    .map(
      (c) =>
        `  (${sqlString(c.name)}, ${sqlString(c.district)}, ${fmtCoord(c.lat)}, ${fmtCoord(c.lng)})`
    )
    .join(",\n");
...
function buildPgTap(courts) {
  const n = courts.length;
  const legacySix = LEGACY_SIX.map((name) => sqlString(name)).join(",");
