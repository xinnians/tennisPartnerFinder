...
test.beforeEach(async ({ page }) => {
  ...
  await page.addInitScript(() => {
    globalThis.__surfaceRootUnmounts = [];
    globalThis.__tennisE2ETestHooks = {
      ...(globalThis.__tennisE2ETestHooks ?? {}),
      surfaceRootLifecycle: {
        onUnmount: (surfaceId) => globalThis.__surfaceRootUnmounts.push(surfaceId),
      },
    };
  });
});

test("closing and replacing sheets unmount each React root exactly once", async ({ page }) => {
  const pageErrors = [];
  ...
  await expect.poll(() => page.evaluate(() => globalThis.__surfaceRootUnmounts)).toEqual(["session-create-modal"]);

  await page.locator("#filters-sheet [data-surface-close]").click();
  await expect(page.locator("#filters-sheet")).toHaveCount(0);
  await expect
    .poll(() => page.evaluate(() => globalThis.__surfaceRootUnmounts))
...
test("a pending join result cannot render after its detail sheet unmounts", async ({ page }) => {
  ...
  await expect.poll(() => page.evaluate(() => globalThis.__surfaceRootUnmounts)).toEqual(["session-sheet"]);
