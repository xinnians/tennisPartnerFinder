...
const GENERATED_DATABASE_TYPES_SUFFIX = "/src/data/databaseTypes.ts";
...
async function readRetiredContactSources() {
  ...
  return sources.filter(({ path }) => !path.endsWith(GENERATED_DATABASE_TYPES_SUFFIX));
}
...
test("frontend source scan allows only the frozen LINE RPC parameter", async () => {
  ...
  const generatedDatabaseTypes = await readFile(new URL("../src/data/databaseTypes.ts", import.meta.url), "utf8");
  const srcSources = sources.filter(({ path }) => /\/src\//.test(path));
  const publicSources = sources.filter(({ path }) => /\/public\//.test(path));
  assert.match(
    generatedDatabaseTypes,
    /^\/\/ Generated by `npm run db:gen-types`\. Do not edit manually\./,
    "the schema-only exclusion must remain an explicitly generated declaration file"
  );
  assert.ok(srcSources.length >= 15, "the frontend source scan must inspect the src/ tree");
  assert.ok(
  ...
  assert.equal(
    sourceCodeMatches(sources, /session_contacts/).length,
    0,
    "frontend JS must not contain session_contacts"
  );
  assert.equal(sourceCodeMatches(sources, /lineId/).length, 0, "frontend JS must not contain lineId");
  assert.equal(
    sourceTextMatches(sources, /line[.]me/i).length,
    0,
...
test("private roster and profile mappers stay separate from public summaries", () => {
  ...
  assert.deepEqual(
    sortedKeys(roster),
    ["sessionId", "participantId", "profileId", "nickname", "ntrp", "playTypes", "homeCourts", "role", "status"].sort()
  );
  ...
  assert.deepEqual(
    sortedKeys(profile),
    ["nick", "ntrp", "types", "courts", "slots", "isPublic", "sharePresence", "openToGreeting"].sort()
  );
...
test("messagesFromGroups keeps only accepted, non-cancelled/expired sessions and sorts by start time", () => {
  ...
  const playedAccepted = {
    ...base,
    sessionId: 3,
    startAt: "2099-01-03T00:00:00Z",
    status: "played",
    viewerParticipantStatus: "accepted",
  };
  const cancelledAccepted = {
    ...base,
    sessionId: 4,
    startAt: "2099-01-04T00:00:00Z",
    status: "cancelled",
    viewerParticipantStatus: "accepted",
  };
...
test("private-profile eligibility enforces each nickname and NTRP boundary from current profile fields", () => {
  ...
  for (const [ntrpValue, expected] of cases) {
    ...
    assert.equal(
      eligibility.ntrpValue,
      expected ? Number(ntrpValue) : null,
      `private NTRP value for ${String(ntrpValue)}`
    );
  }
...
test("player directory mapper keeps its exact public allowlist", () => {
  ...
  assert.deepEqual(
    sortedKeys(mapPlayerDirectoryRow(directoryRow)),
    [
      "profileId",
      "nickname",
      "ntrp",
...
test("session filters use Taipei local dateKey, open-interval band overlap, and preserve source order", () => {
  ...
  assert.deepEqual(
    narrowlyFiltered.map((item) => item.sessionId),
    [1]
  );
  ...
  assert.deepEqual(
    overlapAndUnspecified.map((item) => item.sessionId),
    [2, 3]
  );
  ...
  assert.deepEqual(
    allUpcoming.map((item) => item.sessionId),
    [1, 2, 3, 6]
  );
});
...
test("drawer sorting places ongoing sessions with vacancies first and sinks full sessions last", () => {
  ...
  const sessions = [
    ...
    session({
      ...
      courtLat: 25.0301,
      courtLng: 121.5401,
      startAt: "2026-07-16T23:40:00.000Z",
      slotsRemaining: 0,
    }),
  ];
  ...
  assert.deepEqual(
    sortSessionsForDrawer(sessions, null, now).map((item) => item.sessionId),
    [4, 3, 1, 2, 5]
  );
  assert.deepEqual(
...
test("drawer distance sorting anchors undecided candidates to their nearest catalogue court", () => {
  ...
  assert.deepEqual(
    sorted.map(({ sessionId }) => sessionId),
    [2, 1]
  );
});
...
test("pending intents persist only the approved session-action shapes", () => {
  ...
  storage.setItem(
    PENDING_SESSION_INTENT_KEY,
    JSON.stringify({ action: "join", sessionId: 4, location: { lat: 1, lng: 2 } })
  );
    ...
    client: {
      rpc: async () => ({
        ...
        error: {
          ...
          details: "user text says BLOCKED and SESSION_EXPIRED",
        },
      }),
    },
  });
...
test("Supabase error decoding accepts only exact P0001 session action messages", async () => {
  const cases = [
    ...
    [
      "action only in hint",
      { code: "P0001", message: "database validation failed", hint: "BLOCKED" },
      "UNKNOWN_ACTION_ERROR",
    ],
    [
      "action only in details",
      { code: "P0001", message: "database validation failed", details: "BLOCKED" },
      "UNKNOWN_ACTION_ERROR",
    ],
    [
      "actions only in hint and details",
      { code: "P0001", message: "database validation failed", hint: "SESSION_EXPIRED", details: "BLOCKED" },
      "UNKNOWN_ACTION_ERROR",
    ],
    [
      "player block FK collision",
      { code: "23503", message: "player_blocks_blocked_profile_id_fkey" },
      "UNKNOWN_ACTION_ERROR",
    ],
  ];
...
test("createSession maps the candidates venue contract and both requested NTRP outcomes", async () => {
  ...
  assert.deepEqual(await api.requestToJoinSession(82), {
    outcome: "OK_NTRP_MISSING",
    accepted: false,
    reloadRequired: false,
  });
  assert.deepEqual(await api.requestToJoinSession(83), {
    outcome: "OK_NTRP_OUT_OF_RANGE",
    accepted: false,
    reloadRequired: false,
  });
});
...
test("updateSession and decideSessionCourt use only their Stage 2 RPC contracts", async () => {
  ...
  assert.deepEqual(await api.decideSessionCourt("92", "103", "2026-09-01T02:30:00.000Z"), {
    outcome: "OK",
    reloadRequired: false,
  });
  assert.deepEqual(calls, [
...
test("browser source keeps raw lifecycle, chat, block, and report tables outside all browser access", async () => {
  const source = (await readRetiredContactSources()).map(({ source: fileSource }) => fileSource).join("\n");
