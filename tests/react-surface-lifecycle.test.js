...
test("all 14 React sheet adapters return a tracked lifecycle and register unmount", () => {
  const sheetSources = readdirSync(SHEETS_DIR)
    .filter((name) => name.endsWith(".tsx"))
    .map((name) => ({ name, source: readFileSync(join(SHEETS_DIR, name), "utf8") }))
    .filter(({ source }) => source.includes("createSurfaceRoot("));

  assert.equal(sheetSources.length, 14);
  for (const { name, source } of sheetSources) {
    assert.doesNotMatch(source, /createRoot\(/, `${name} bypasses the tracked surface lifecycle`);
    assert.match(source, /\breturn reactRoot;|\bunmount: reactRoot\.unmount/, `${name} hides its unmount contract`);
  }
...
test("Session Detail blocks both direct and async commits after its surface dies", () => {
  ...
  assert.match(detail, /if \(!reactRoot\.isSurfaceRootLive\(\)\) return;/);
